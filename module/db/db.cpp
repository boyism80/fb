...
bool fb::db::query(const std::vector<std::string>& queries)
{
    if(_context == nullptr)
        return false;
...
bool fb::db::query(std::function<void(daotk::mysql::connection&, std::vector<daotk::mysql::result>&)> callback, const std::vector<std::string>& queries)
{
    if(_context == nullptr)
        return false;
