...
// clang-format off
uint32_t fb::crypto::HEX_TABLE[10][0x100] = 
{
...
fb::crypto::crypto() :
    crypto(0, (uint8_t*)"NexonInc.")
{ }

fb::crypto::crypto(uint8_t types, const uint8_t* key) :
    _type(types)
{
...
fb::crypto::crypto(const crypto& crt) :
    crypto(crt._type, crt._key)
{ }
...
void fb::crypto::crypt(const uint8_t* src, uint8_t* dst, uint32_t size, const uint8_t* key, uint32_t ksize)
{
    auto current_src = src;
    auto current_dst = dst;
    auto num_loop    = uint32_t(size / 4);
...
uint32_t fb::crypto::encrypt(fb::stream& data, uint32_t offset, uint32_t size)
{
...
uint32_t fb::crypto::encrypt(fb::stream& data)
{
    if (data.size() == 0)
        return -1;
...
uint32_t fb::crypto::decrypt(fb::stream& data, uint32_t offset, uint32_t size)
{
    auto extended_size = size + 0x100;
    auto buffer_src    = (uint8_t*)data.data() + offset;
    auto buffer_dst    = new uint8_t[extended_size];
...
uint32_t fb::crypto::decrypt(fb::stream& data)
{
    if (data.size() == 0)
        return -1;
...
uint32_t fb::crypto::wrap(fb::stream& data, uint32_t offset) const
{
...
uint32_t fb::crypto::wrap(fb::stream& data) const
{
    return this->wrap(data, 0);
}

uint32_t fb::crypto::unwrap(fb::stream& data, uint32_t offset) const
{
    uint16_t size = uint16_t(data.size() - offset);
    if (size < 3)
        return -1;
...
uint32_t fb::crypto::unwrap(fb::stream& data) const
{
    return this->unwrap(data, 0);
}

uint8_t fb::crypto::type() const
{
    return this->_type;
}
...
fb::crypto& fb::crypto::operator= (const crypto& crt)
{
    for (int i = 0; i < 4; i++)
...
fb::crypto fb::crypto::generate()
{
    ...
    return crypto(enc_type, enc_key);
}

bool fb::crypto::validate(uint8_t type, const uint8_t* key, uint8_t ksize)
{
    if (type > KEY_SIZE)
        return false;
    ...
    if (std::any_of(key, key + KEY_SIZE, [](uint8_t x) {
            return x == 0;
        }))
        return false;
