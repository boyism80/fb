...
fb::buffer::buffer(const buffer& right) :
    fb::buffer((uint8_t*)right.data(), right.size())
{ }
...
fb::istream::istream(size_t size) :
    buffer(size)
{ }

fb::istream::istream(const uint8_t* data, size_t size) :
    buffer(data, size)
{ }
...
{
#ifdef _WIN32
    int32_t value = (*this)[this->_offset + 0] << 0 | (*this)[this->_offset + 1] << 8 | (*this)[this->_offset + 2] << 16 | (*this)[this->_offset + 3] << 24;
    if (endian == fb::buffer::endian::BIG)
        value = _byteswap_ulong(value);
#else
    int32_t value = (*this)[this->_offset + 0] << 24 | (*this)[this->_offset + 1] << 16 | (*this)[this->_offset + 2] << 8 | (*this)[this->_offset + 3] << 0;
    if (endian == fb::buffer::endian::LITTLE)
...
fb::ostream::ostream(size_t size) :
    buffer(size)
{ }

fb::ostream::ostream(const uint8_t* data, size_t size) :
    buffer(size)
{
