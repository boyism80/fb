...
async::task<bool> attack_test::attack_scenario_1()
{
    ...
    auto spell_index = co_await attacker->learn_spell("유인", DEFAULT_TIMEOUT);
    ...
    std::ignore = co_await attacker->set_max_hp_mp(100000, 100000, DEFAULT_TIMEOUT);
...
async::task<bool> attack_test::attack_scenario_2()
{
    ...
    std::ignore = co_await target->set_max_hp_mp(100000, 100000, DEFAULT_TIMEOUT);
