...
async::task<bool> bulletin_test::get_sections_scenario()
{
    ...
    fb::logger::debug("Testing getting bulletin sections");
...
async::task<bool> bulletin_test::bulletin_write_scenario()
{
    ...
    fb::logger::debug("Testing bulletin write with low level (should fail)");
    ...
    fb::logger::debug("Bulletin write failed as expected with low level");
    bot1->chat("=== BULLETIN WRITE FAILED AS EXPECTED (LOW LEVEL) ===");

    // Change level to 10 for both bots
    fb::logger::debug("Changing level to 10 for both bots");
    co_await bot1->change_level(10, DEFAULT_TIMEOUT);
    co_await bot2->change_level(10, DEFAULT_TIMEOUT);

    fb::logger::debug("Level changed to 10, testing bulletin write again");
    ...
    fb::logger::debug("Bot1 write test successful with level 10");
...
async::task<bool> bulletin_test::bulletin_read_scenario()
{
    ...
    fb::logger::debug("Found our written article - ID: {}", found_article_id);
    bot1->chat(std::format("=== FOUND OUR WRITTEN ARTICLE - ID: {} ===", found_article_id));

    // Test reading our article
    fb::logger::debug("Testing bulletin article read functionality");
    ...
    {
        fb::logger::debug("Could not find an article from another user to test. Skipping test.");
    ...
    else
    {
        ...
        fb::logger::debug("Testing to read another user's article");
...
async::task<bool> bulletin_test::mail_scenario()
{
    ...
    fb::logger::debug("Sending mail from {} to {}", bot1->name(), bot2->name());
    ...
    fb::logger::debug("Mail found with ID: {}", mail_id);

    fb::logger::debug("Reading mail {}", mail_id);
    ...
    fb::logger::debug("Deleting mail {}", mail_id);
    ...
    fb::logger::debug("Mail deleted successfully");

    // Test deleting a non-existent mail
    fb::logger::debug("Testing deleting a non-existent mail (should fail)");
...
async::task<std::vector<fb::bot::bulletin>> bulletin_test::get_sections(std::shared_ptr<game_bot> bot)
{
    if (bot == nullptr)
        co_return {};
    ...
    fb::logger::debug("Bulletin sections retrieved successfully - Sections: {}", resp.bulletins.size());

    co_return resp.bulletins;
}

async::task<std::vector<fb::bot::integration::bulletin_bot::article_data>>
bulletin_test::get_articles(std::shared_ptr<game_bot> bot, uint16_t section, uint16_t offset)
{
    if (bot == nullptr)
        co_return {};
    ...
    fb::logger::debug("Bulletin articles retrieved successfully - Section: {}, Name: {}, Articles: {}",
...
async::task<bool> bulletin_test::delete_article(std::shared_ptr<game_bot> bot, uint16_t section, uint16_t article_id)
{
    if (bot == nullptr || article_id == 0)
        co_return false;
...
bulletin_test::get_mails(std::shared_ptr<game_bot> bot)
{
    if (bot == nullptr)
        co_return {};
    ...
    // Send the request and wait for response
    auto&& resp = co_await bot->request<fb::bot::integration::bulletin_bot>(
        game_reqs::bulletin(BULLETIN_ACTION::ARTICLES, 0xFFFF, 0, 0x7FFF), // section 0xFFFF for mail
        [](auto& resp) -> bool {
            switch (resp.type)
            {
    ...
    fb::logger::debug("Mails retrieved successfully - Mails: {}", resp.mails.size());
...
async::task<bool> bulletin_test::delete_mail(std::shared_ptr<game_bot> bot, uint16_t mail_id)
{
    if (bot == nullptr)
        co_return false;
