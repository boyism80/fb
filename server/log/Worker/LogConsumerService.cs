...
namespace Log.Worker
{
    ...
    public class LogConsumerService : BackgroundService
    {
        ...
        private async Task ConnectToRabbitMQAsync(CancellationToken cancellationToken)
        {
            ...
            // Declare a single auto-generated queue for this consumer instance
            // This allows multiple LogConsumerService instances to work in parallel
            var queueDeclareResult = _channel.QueueDeclare(
                queue: string.Empty, // Empty queue name = auto-generated by RabbitMQ
                durable: true,
                exclusive: false,
                autoDelete: false,
                arguments: null);

            var autoGeneratedQueueName = queueDeclareResult.QueueName;
            _queueNames.Add(autoGeneratedQueueName);

            _logger.LogInformation($"Created auto-generated queue: {autoGeneratedQueueName}");

            // Bind this queue to all log routing keys (fb.log.0 to fb.log.{queueSize-1})
            // This allows this consumer to receive messages from all log routes
            for (int i = 0; i < queueSize; i++)
            {
                ...
                try
                {
                    _channel.QueueBind(autoGeneratedQueueName, ExchangeName, routingKey);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to bind queue '{QueueName}' to routing key '{RoutingKey}'", autoGeneratedQueueName, routingKey);
                    throw;
                }
            }

            _logger.LogInformation($"Connected to RabbitMQ and bound queue '{autoGeneratedQueueName}' to {queueSize} log routing keys");
