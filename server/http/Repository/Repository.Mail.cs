...
namespace Http.Reepository
{
    ...
    public class MailRepository : IRepository
    {
        ...
        public async Task<List<Mail>> GetList(string section, uint user, ushort offset, ushort count)
        {
            await using var conn = _dbContext.Connection(section, user);
        ...
        public async Task<Mail> Get(string section, uint user, uint id)
        {
            await using var conn = _dbContext.Connection(section, user);
            var dynamicParams = new DynamicParameters();
            dynamicParams.Add("user", user);
        ...
        public async Task<Mail> Write(string section, string user, uint sender, string title, string contents)
        {
            uint uid;
            ...
            else
            {
                ...
                uid = await _dbContext.Character.GetCharacterId(section, user) ??
                    throw new LogicException(ErrorCode.NotFoundCharacter);
            }

            await using var conn = _dbContext.Connection(section, uid);
            var dynamicParams = new DynamicParameters();
        ...
        public async Task<ushort> Unread(string section, uint user)
        {
            await using var conn = _dbContext.Connection(section, user);
        ...
        public async Task<bool> Delete(string section, uint user, uint id)
        {
            await using var conn = _dbContext.Connection(section, user);
