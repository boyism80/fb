    ...
    _redis(config<std::string>("redis:ip").c_str(), config<uint16_t>("redis:port"), config<uint32_t>("redis:pool"))
{
    ...
    this->handler.protocol.bind(&server::handle_login);
    this->handler.protocol.bind(&server::handle_agreement);
    this->handler.protocol.bind(&server::handle_create_account);
    this->handler.protocol.bind(&server::handle_complete);
    this->handler.protocol.bind(&server::handle_change_password);
}
...
async::task<bool> server::handle_agreement(fb::socket<session>& socket, const request::agreement& request)
{
    try
    {
...
async::task<bool> server::handle_create_account(fb::socket<session>& socket, const request::create& request)
{
    // 여기는 task handler
    auto fd   = socket.fd();
...
async::task<bool> server::handle_complete(fb::socket<session>& socket, const request::complete& request)
{
    auto fd   = socket.fd();
    auto weak = socket.weak_from_this_as<fb::socket<session>>();
...
async::task<bool> server::handle_login(fb::socket<session>& socket, const request::login& request)
{
    auto weak  = socket.weak_from_this_as<fb::socket<session>>();
...
async::task<bool> server::handle_change_password(fb::socket<session>& socket, const request::update_pw& request)
{
    auto weak = socket.weak_from_this_as<fb::socket<session>>();
    try
    {
