...
async::task<void> rezen::spawn(std::thread::id thread_id)
{
    ...
    for (int i = 0; i < spawn_count; i++)
    {
        ...
        auto mob = this->_server.make<fb::game::mob>(fb::model::table::mob[this->model.mob], mob::initial_params{.alive = true, .rezen = this});
    ...
    if (owner == nullptr && !model.drop.empty())
    {
        ...
        for (auto& dsl : drop.dsl)
        {
            ...
            case DSL::item:
            {
                ...
                auto item   = fb::model::table::item[params.id].make(this->server);
                std::ignore = co_await item->map(map, position, DESTROY_TYPE::DEFAULT, false);
                oids.push_back(item->oid());
            }
            break;
