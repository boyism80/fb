...
async::task<void> server::join_clan_member(const clan& clan, character& inviter, character& invitee)
{
    auto inviter_weak = inviter.weak_from_this_as<character>();
    auto invitee_weak = invitee.weak_from_this_as<character>();

    if (invitee.clan_id().has_value())
        throw std::runtime_error(_TEXT(MESSAGE_ALREADY_JOINED_CLAN));

    auto member = clan.member(inviter.name());
    if (member == nullptr)
        throw std::runtime_error(_TEXT(MESSAGE_NOT_JOINED_CLAN));

    co_await this->characters.async_write([this, &clan, &inviter, &invitee](auto& characters) -> async::task<void> {
        if (invitee.clan_id().has_value())
            co_return;

        auto weak    = invitee.template weak_from_this_as<character>();
        auto members = std::vector<std::shared_ptr<fb::game::character>>();
        for (auto& [_, weak_ptr] : clan.characters())
        {
            auto shared_ptr = weak_ptr.lock();
        ...
        characters.foreach_enqueue(
            [this, &invitee](auto& member) -> async::task<void> {
                member->message(std::format(_TEXT(MESSAGE_CLAN_MEMBER_JOINED), invitee.name()), MESSAGE_TYPE::NOTIFY);
                co_return;
            },
            members);
        ...
        if (weak.expired() == false)
        {
            const_cast<clan&>(clan).attach(weak);
            invitee.clan_id(clan.id());
            invitee.update_external(false);
            invitee.message(std::format(_TEXT(MESSAGE_CLAN_JOINED_SUCCESS), clan.name()), MESSAGE_TYPE::NOTIFY);

            auto log_data              = Json::Value();
            log_data["character_id"]   = static_cast<Json::Int64>(invitee.id);
            log_data["character_name"] = UTF8(invitee.name(), PLATFORM::WINDOWS);
            log_data["clan_id"]        = static_cast<Json::Int64>(clan.id());
            log_data["clan_name"]      = UTF8(clan.name(), PLATFORM::WINDOWS);
            this->log.write("clan_join", log_data);
        }
...
async::task<void> server::leave_clan_member(const clan& clan, std::string_view name)
{
    auto name_str = std::string(name);
    co_await this->characters.async_write([this, &clan, name_str](auto& characters) -> async::task<void> {
        auto ch = characters.find(name_str);
        if (ch == nullptr)
            co_return;

        auto weak = ch->template weak_from_this_as<character>();
        const_cast<clan&>(clan).detach(weak);
        ch->clan_reset();
        ch->update_external(false);
        ch->message(_TEXT(MESSAGE_CLAN_LEFT), MESSAGE_TYPE::NOTIFY);

        auto log_data              = Json::Value();
        log_data["character_id"]   = static_cast<Json::Int64>(ch->id);
        log_data["character_name"] = UTF8(name_str, PLATFORM::WINDOWS);
        log_data["clan_id"]        = static_cast<Json::Int64>(clan.id());
        log_data["clan_name"]      = UTF8(clan.name(), PLATFORM::WINDOWS);
        this->log.write("clan_leave", log_data);

        const_cast<clan&>(clan).leave(name_str);
        auto members = std::vector<std::shared_ptr<fb::game::character>>();
        for (auto& [_, weak_ptr] : clan.characters())
        {
            auto shared_ptr = weak_ptr.lock();
        ...
        auto message = std::format(_TEXT(MESSAGE_CLAN_MEMBER_LEFT), name_str);
        characters.foreach_enqueue(
            [this, message](auto& member) -> async::task<void> {
                member->message(message, MESSAGE_TYPE::NOTIFY);
                co_return;
