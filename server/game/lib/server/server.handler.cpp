...
async::task<bool> server::handle_login(fb::socket<character>& socket, const fb_reqs::login& request)
{
    auto ch = socket.data();
    if (ch == nullptr)
        co_return false;
...
async::task<bool> server::handle_direction(fb::socket<character>& socket, const fb_reqs::direction& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_logout(fb::socket<character>& socket, const fb_reqs::exit& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_move(fb::socket<character>& socket, const fb_reqs::move& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_update_move(fb::socket<character>& socket, const fb_reqs::update_move& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_attack(fb::socket<character>& socket, const fb_reqs::attack& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_loot(fb::socket<character>& socket, const fb_reqs::loot& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_emotion(fb::socket<character>& socket, const fb_reqs::emotion& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_update_map(fb::socket<character>& socket, const fb_reqs::map_update& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_update_screen(fb::socket<character>& socket, const fb_reqs::update_screen& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_active_item(fb::socket<character>& socket, const fb_reqs::item_active& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_inactive_item(fb::socket<character>& socket, const fb_reqs::item_inactive& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_drop_item(fb::socket<character>& socket, const fb_reqs::item_drop& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_drop_money(fb::socket<character>& socket, const fb_reqs::item_drop_money& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_front_info(fb::socket<character>& socket, const fb_reqs::front_info& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_self_info(fb::socket<character>& socket, const fb_reqs::self_info& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_option_changed(fb::socket<character>& socket, const fb_reqs::update_option& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_click_object(fb::socket<character>& socket, const fb_reqs::click& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
// TODO : on_item_detail
async::task<bool> server::handle_item_info(fb::socket<character>& socket, const fb_reqs::item_info& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_item_combine(fb::socket<character>& socket, const fb_reqs::item_combine& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_trade(fb::socket<character>& socket, const fb_reqs::trade& request)
{
    auto me = socket.data();
    if (me->inited() == false)
        co_return true;
...
async::task<bool> server::handle_world(fb::socket<character>& socket, const fb_reqs::map_world& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_object_miss(fb::socket<character>& socket, const fb_reqs::miss& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_give_item(fb::socket<character>& socket, const fb_reqs::give_item& request)
{
    auto me = socket.data();
    if (me->inited() == false)
        co_return true;
...
async::task<bool> server::handle_give_money(fb::socket<character>& socket, const fb_reqs::give_money& request)
{
    auto me = socket.data();
    if (me->inited() == false)
        co_return true;
...
async::task<bool> server::handle_post(fb::socket<character>& socket, const fb_reqs::post& request)
{
    co_return true;
}

async::task<bool> server::handle_group(fb::socket<character>& socket, const fb_reqs::group& request)
{
    auto me = socket.data();
    if (me->inited() == false)
        co_return true;
...
async::task<bool> server::handle_user_list(fb::socket<character>& socket, const fb_reqs::user_list& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_chat(fb::socket<character>& socket, const fb_reqs::chat& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_bulletin(fb::socket<character>& socket, const fb_reqs::bulletin& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_swap(fb::socket<character>& socket, const fb_reqs::swap& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_dialog(fb::socket<character>& socket, const fb_reqs::dialog& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_throw_item(fb::socket<character>& socket, const fb_reqs::item_throws& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_spell(fb::socket<character>& socket, const fb_reqs::spell_cast& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_door(fb::socket<character>& socket, const fb_reqs::door& request)
{
    auto ch = socket.data();
    if (ch->inited() == false)
        co_return true;
...
async::task<bool> server::handle_whisper(fb::socket<character>& socket, const fb_reqs::whisper& request)
{
    auto me = socket.data();
    if (me->inited() == false)
        co_return true;
