...
{
    me.send(fb_resp::dialog(me.server.model, object, message, button_prev, button_next, oid));
}
...
{
    me.send(fb_resp::dialog_menu(me.server.model, obj, menus, message, oid));
}
...
{
    me.send(fb_resp::dialog_list(me.server.model, obj, menus, message, button_prev, oid));
}
...
{
    me.send(fb_resp::dialog_slot(me.server.model, obj, item_slots, message, oid));
}
...
{
    me.send(fb_resp::dialog_item(me.server.model, obj, pairs, message, oid, pursuit));
}

void listener_impl::on_dialog(character& me, const fb::model::object& obj, const std::string& message, uint32_t oid)
{
    me.send(fb_resp::dialog_input(me.server.model, obj, message, oid));
}
...
void listener_impl::on_dialog(character&               me,
                              ...
                              uint32_t                 oid)
{
    me.send(fb_resp::dialog_input_ext(me.server.model, obj, message, top, bottom, maxlen, prev, oid));
}