...
{
    std::ignore = this->server.send(me, fb_resp::action(me, action, duration), scope::PIVOT);
}
...
{
    std::ignore = this->server.send(me, fb_resp::update_hp(me, diff, critical), scope::PIVOT);
}