...
::ON_ACTIVATED_10208_003::
    button = me:dialog(model, '-궐기한지 27일째\n천황이 나의 반역 사실을 눈치채고 대비하기 전에\n최대한 빨리 다른 영지를 무력화시킨다는 작전은 주효했다.\n대부분의 영지는 무력화시키거나 우리 편으로 만들었고, 남은 영지들은 뿔뿔이 흩어져 있어 천황이 남은 영지를 규합하여 우리 군에 대항하기도 힘들 것이다.', { prev = true, next = true })
    if button == DIALOG_RESULT.QUIT then
        return
    end
...
::ON_ACTIVATED_10208_004::
    button = me:dialog(model, '이제 천하통일의 그날까지 얼마 남지 않았다. 하지만 어쩐지 불안한 것은 왜일까? 좋지 않은 예감이 든다...', { prev = true, next = true })
    if button == DIALOG_RESULT.QUIT then
        return
    end
...
::ON_ACTIVATED_10208_005::
    button = me:dialog(model, '-궐기한지 31일째\n예감은 적중했다.\n천황은 남은 영지에 밀사를 보내 연합군을 조직, 우리를 사방에서 포위하고 공격해왔다.\n천황의 지독한 폭정에 모두 반발하고 있었으니, 천황의 명을 따르지 않을 것이라고 생각했건만. 분명 많은 부와 더 좋은 지위를 미끼로 유혹했겠지...', { prev = true, next = true })
    if button == DIALOG_RESULT.QUIT then
        return
    end
...
::ON_ACTIVATED_10208_006::
    button = me:dialog(model, '일기를 쓰고 있을 시간이 없다.\n오늘 밤에 있을 전투가 우리 군의 미래를 결정할 것이다.', { prev = true, next = true })
    if button == DIALOG_RESULT.QUIT then
        return
    end
