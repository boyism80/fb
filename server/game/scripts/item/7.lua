...
return {
    on_activated = function(me, item)
        ...
        if q == nil or q:completed() then
            ...
            ::ON_ACTIVATED_7_0001::
            btn = me:dialog(model, '<힌트>\n 레벨 5를 만들면 직업을 선택하실 수 있습니다.', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            if q == nil then
                q = me:start_quest(quest.QUEST_NAKRANG7)
                if q == nil then
                    return
                end
            end
        ...
        if me:level() >= GOAL_LEVEL then
            btn = me:dialog(model, '잘 하셨습니다!! 상으로\n초심자의방패를 드릴게요!!', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ...
            q:complete()
            me:dialog(model, '<보상>\n\'초심자의방패\',\n\'낙랑의두루마리8\'을 얻다!!!', { prev = false, next = true })
        else
            me:dialog(model, '레벨 5를 만드는 것은 그다지\n힘들지 않습니다. 왕초보\n사냥터에서만 사냥해도 금방\n만드실 수 있을 꺼에요.', { prev = false, next = true })
        end
    end,
