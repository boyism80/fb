...
return {
    on_activated = function(me, item)
        ...
        if squirrel >= GOAL and rabbit >= GOAL then
            ::ON_ACTIVATED_5_0002::
            btn = me:dialog(model, '잘 하셨습니다!! 상으로\n초심자의머리띠를 드릴게요!!', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ...
            if give_exp then
                btn = me:dialog(model, '<보상>\n\'초심자의귀마개\'\n\'낙랑의두루마리6\'을 얻다!!!\n경험치 200 상승!!', { prev = false, next = true })
            else
                btn = me:dialog(model, '<보상>\n\'초심자의귀마개\'\n\'낙랑의두루마리6\'을 얻다!!!', { prev = false, next = true })
            end
            if btn == DIALOG_RESULT.QUIT then
                return
            end
        ...
        ::ON_ACTIVATED_5_0003::
        btn = me:dialog(model, string.format('사냥한 토끼 : %d / %d\n사냥한 다람쥐 : %d / %d', rabbit, GOAL, squirrel, GOAL), { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::ON_ACTIVATED_5_0004::
        btn = me:dialog(model, '몬스터 앞으로 가서\n스페이스바를 연타하거나,\n마우스로 몬스터를 클릭하면\n공격할 수 있습니다.', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
        ...
        btn = me:dialog(model, '귀엽지만.. \n눈물을 머금고\n조금만 더 사냥해보세요..', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
