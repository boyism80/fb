...
function ON_ACTIVATED_4(me, item)
    ...
    if not HUNTING_MAPS[map_name] then
        ::ON_ACTIVATED_4_0002::
        btn = me:dialog(model, '일단 주막에서 나오셔서,\n화면의 왼쪽 위의 조그만\n사각형 버튼을 누르시면\n미니맵이 나옵니다.(단축키\n*), 이것을 참고하셔서\n\'왕초보사냥터\'로 찾아오세요.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        btn = me:dialog(model, '왕초보사냥터 안에 들어오셔서 두루마리를 사용하셔야 합니다.', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
    ...
    ::ON_ACTIVATED_4_0003::
    btn = me:dialog(model, '잘 찾아오셨네요!!!', { prev = false, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    btn = me:dialog(model, '잘 하셨습니다!! 상으로\n노란비서를 드릴게요!!', { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if btn == DIALOG_RESULT.PREV then
    ...
    if give_exp then
        me:dialog(model, '<보상>\n\'노란비서\',\n\'낙랑의두루마리5\'을 얻다!!!\n경험치 100 상승!!', { prev = false, next = true })
    else
        me:dialog(model, '<보상>\n\'노란비서\',\n\'낙랑의두루마리5\'을 얻다!!!', { prev = false, next = true })
    end
