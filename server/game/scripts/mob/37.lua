...
function ON_MOB_KILL_37(me, mobs)
    ...
    local btn = me:dialog(mob, '당연히 아무도 모르는 곳에 잘 숨겨 두었소. 음.. 하지만 다른분도 아닌 용왕님께서 병이 위중하시다 하니, 내 기꺼이 내 간을 용왕님께 드리리다.', false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end

    sel, list_btn = me:list(mob, '어떻소?', { '좋아, 그럼 내 여기서 기다리마', '흥, 내가 속을줄 아느냐?' }, false)
    if sel == nil then
        return
    end
