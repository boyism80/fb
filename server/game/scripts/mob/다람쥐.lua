-- Squirrel kill count for 낙랑의두루마리5 (quest 228); param "squirrel,rabbit", max 5 each.
...
-- @param[in]  me   The mob that died (squirrel).
...
-- @brief   Increment squirrel count in QUEST_NAKRANG5 param "squirrel,rabbit" (step 1 only).
-- @param[in]  you  The character who killed the mob.
local function run_nakrang5_squirrel(you)
    ...
    local squirrel = math.min((tonumber(a) or 0) + 1, GOAL)
    local rabbit = math.min(tonumber(b) or 0, GOAL)
    q5:param(string.format('%d,%d', squirrel, rabbit))
end

-- @brief   Increment squirrel count in QUEST_BEGINNER_PATH param "squirrel,rabbit" (step 5 only, 사도).
-- @param[in]  you  The character who killed the mob.
local function run_beginner_path_squirrel(you)
...
-- @brief   Squirrel mob death: update NAKRANG_HUNT, NAKRANG5, and BEGINNER_PATH (사도) kill counts.
-- @param[in]  me   The squirrel mob that died.
-- @param[in]  you  The character who killed the mob.
function ON_MOB_DIE_1(me, you)
    ...
    run_nakrang5_squirrel(you)
    run_beginner_path_squirrel(you)
end