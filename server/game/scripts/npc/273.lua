...
function NPC_273(me, npc)
    ...
    if q == nil then
        ...
        ::NPC_273_0012::
        sel, btn = me:list(npc, "누가 가져갔는지, 아니면 제가 어디에 떨어뜨렸는지 통 보이질 않네요. 으으... 그 중요한 물건을 잃어버린 것을 스승님께 들키는 날이면...", { "저도 찾는 것을 돕겠습니다.", "그러게 평소에 조심하셨어야죠. 전 이만..." }, false)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ...
        ::NPC_273_0022::
        me:dialog(npc, "아직 스승님께 들키진 않았습니다만 만약 들키기라도 하면... 으으, 불안하군요. 일단 이 부근부터 좀 뒤져 주십시오.", false, false)
        return
    end
    ...
    if step == 4 then
        ...
        ::NPC_273_0031::
        button = me:dialog(npc, "사례로 '마반경'을 드리죠. 이것은 사악한 힘으로부터 몸을 지켜주는 신기한 물건입니다.", false, true)
        if button == DIALOG_RESULT.PREV then
            goto NPC_273_0030
        end
        if button == DIALOG_RESULT.QUIT then
        ...
        me:dialog(npc, "어이쿠, 스승님께 들키키 전에 파마의피리를 원래 자리에다 돌려놔야죠. 그럼 전 이만...!", false, false)
        return
    end
