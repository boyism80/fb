...
local function craft_amber_star_exchange(me, ch, exchange_list)
    local list_options = {}
    for _, e in ipairs(exchange_list) do
        list_options[#list_options + 1] = e.dest.name .. '입니다.'
    end
    local choice = me:list(ch, '어떤 색깔의 호박별을 만드시겠어요?', list_options)
    if choice == nil then
    ...
    if code == enum.EXCHANGE_RESULT.LACK_COST then
        return me:dialog(ch, name_with(src.name, '이', '가') .. ' 부족합니다.', false, true)
    end
    if code == enum.EXCHANGE_RESULT.LACK_CAPACITY then
        return me:dialog(ch, '소지품이 가득 차서 ' .. name_with(dest.name, '을', '를') .. ' 받을 수 없어요.', false, true)
    end
    return me:dialog(ch, name_with(dest.name, '을', '를') .. ' 만들어드렸습니다.', false, true)
end
...
function NPC_233(me, npc_obj)
    npc.shop(me, npc_obj, {
        ...
        menu = {
            ...
            { '연호박별만들기', function(me, ch)
                return craft_amber_star_exchange(me, ch, AMBER_STAR_EXCHANGE)
            end },
        },
    })
end