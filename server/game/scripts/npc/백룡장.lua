...
local function run_weapon_three_dialogs(me, npc, line1, line2, line3)
    ...
    me:dialog(npc, line3, true, false)
    return true
end
...
local function run_weapon_info_loop(me, npc)
    ...
    while true do
        ...
        if weapon_sel == 0 then
            if not run_weapon_three_dialogs(me, npc,
                ...
                "그대가 만약 강해지고 싶다면 한번 진성검을 만들어보게나...") then
                return
            end
        elseif weapon_sel == 1 then
            if not run_weapon_three_dialogs(me, npc,
                ...
                "그대가 만약 강해지고 싶다면 한번 백진도를 만들어보게나...") then
                return
            end
        elseif weapon_sel == 2 then
            if not run_weapon_three_dialogs(me, npc,
                ...
                "그대가 만약 가장 강한 주술사가 되고 싶다면 한번 만들어보게나...") then
                return
            end
        elseif weapon_sel == 3 then
            if not run_weapon_three_dialogs(me, npc,
                ...
                "그대가 만약 가장 강한 도사가 되고 싶다면 한번 만들어보게나...") then
                return
            end
        ...
        elseif weapon_sel == 5 then
            ...
            if not run_weapon_three_dialogs(me, npc,
                ...
                "자네가 귀한 술을 가지고, 그 대장장이를 찾아가기만 한다면, 아마도 만들어 줄 지도 모르지...") then
                return
            end
        end
    end
