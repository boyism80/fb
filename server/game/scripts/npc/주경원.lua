function NPC_241(me, npc)
    ...
    if quest == nil then
        ::NPC_241_COS001::
        btn = me:dialog(npc, '안녕하시오. ' .. me:name() .. '씨, 처음 뵙겠소. 난 여기 남양에서 뱀술을 만드는 주경원이라고 하오.', true, true)
        ...
        local sel, lb = me:list(npc, '뱀이 필요한데.. 어떻게 구한담?', { '제가 도와드리죠. 뱀을 구해 드리면 되는 건가요?', '뱀술말고 다른 술을 만드시면 되지 않나요?' })
    ...
    if quest:step() == 1 then
        ...
        ::NPC_241_COS003::
        btn = me:dialog(npc, '오오, 뱀고기 100개와 좋은뱀고기 5개를 정말 구해왔군. 고맙네 고마워.. 이제 뱀술을 한 번 만들어 보겠네.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ...
        local sel = me:list(npc, '이 일을 어쩐다....', { '내친김에 나머지 재료도 제가 구해다 드리지요.', '에이, 이젠 저도 몰라요.' })
        ...
        ::NPC_241_COS004::
        btn = me:dialog(npc, '아아 고맙네, 그럼 일단 뱀고기는 내가 가져가겠네..\n\n나머지 재료는 기린의피와 청심초 5뿌리라네. 기린의피는 붉은색의 기린을 잡아서 피를 받아오면 되네.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
    ...
    if quest:step() == 2 then
        ...
        ::NPC_241_COS005::
        btn = me:dialog(npc, '오오, 고맙네 고마워.. 내 뱀술을 만들어 줄 테니 잠시만 기다리게!', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        btn = me:dialog(npc, '... ... ...\n\n다 됐네! 한 병은 평소에 신세가 많았던 천선도사에게 좀 가져다 드리고, 한병은 자네가 마시게. 허허허.. 정말 고마우이..', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
