--- 1st~4th promotion, promotion skills, and 3차승급 옷 (진인의영혼/진인의심장) implemented.
function NPC_5(me, npc)
    ...
    if sel == 0 then
        if me:promotion() >= 4 then
            ...
            NPC_PROMOTION(me, npc, CLASS.POET)
        end
        return
    end
    if sel == 1 then
        NPC_PROMOTION_SKILLS(me, npc, CLASS.POET)
        return
    end
    if sel == 2 then
        NPC_PROMOTION_CLOTHES(me, npc, CLASS.POET)
    end
end
