...
return {
    on_click = function(me, npc)
        ...
        ::NPC_268_0002::
        button = me:dialog(npc, '같이 온 동료가 있으면 동료와 함께 들여보내 주겠다.', { prev = true, next = false })
        if button == DIALOG_RESULT.QUIT then
            return
        end
        ...
        while true do
            ...
            if me:map() ~= teammate:map() then
                local btn = me:dialog(npc, '다른 맵에 있습니다.', { prev = false, next = true })
                if btn == DIALOG_RESULT.QUIT then
                    return
                end
                goto NPC_268_0010
            ...
            local btn = me:dialog(npc, '동료에게 확인을 받겠다.', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if teammate:script('scripts/npc/268.lua', 'npc_confirm_teammate', me:name(), npc) ~= true then
                local btn = me:dialog(npc, '동료에게 확인을 받지 못했습니다.', { prev = false, next = true })
                if btn == DIALOG_RESULT.QUIT then
                    return
                end
                goto NPC_268_0010
        ...
        local btn = me:dialog(npc, '그럼 보내주도록 하겠다.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
    ...
    npc_confirm_teammate = function(me, teammate_name, npc)
            ...
            if selected == 2 then
                me:dialog(npc, '확인했다. 그럼 잠시 기다리고 있도록 하여라.', { prev = false, next = false })
                return true
            else
                return false
            end
