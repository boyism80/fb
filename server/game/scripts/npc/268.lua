-- npc: 동쪽태극문보초
function NPC_268_CONFIRM_TEAMMATE(me, teammate_name, npc)
    local button = me:dialog(npc, string.format('%s의 동료에게 묻겠다.', teammate_name), false, true)
...
function NPC_268(me, npc)
    ...
    while true do
        ...
        local name = me:input(npc, '동료의 이름은?', '동료의 이름은', '입니다.', 12, false)
        ...
        if teammate:script('scripts/npc/268.lua', 'NPC_268_CONFIRM_TEAMMATE', me:name(), npc) ~= true then
    ...
    me:dialog(npc, '그럼 보내주도록 하겠다.', false, true)
    if not me:rmitem('동문열쇠', 1, ITEM_DELETE_TYPE.GIVE) then
        me:dialog(npc, '열쇠를 가지고 있지 않습니다.', false, true)
        return
    end
