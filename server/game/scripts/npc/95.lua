...
return {
    on_click = function(me, npc)
        ...
        if step == 0 then
            ...
            ::NPC_95_0002::
            local btn = me:dialog(npc, "천기를 살펴본 바로, 이 모든 현상들이 귀결되는 지점이 바로 '반고'의 신전이네. 반고 부활의 조짐일세. 이런 일을 행할 수 있는 이는 오제 뿐이리라네. 신선 세계 일에 관여치 않던 오제가 반고 부활의 의식을 진행하고 있는 것이지. 이 세상은 반고로부터 기인해 존재하게 되었지만, 반고와 현세는 공존할 수 없다네.", { prev = true, next = true })
            ...
            ::NPC_95_0003::
            btn = me:dialog(npc, '반고의 부활로 인해 새로운 천지가 열리게 될 것이세. 바꾸어 말하자면 곧 현세의 종말을 뜻하는 것일세. 갈수록 더러워지고 추악해지는 인간세상을 정화하려는 신의 분노라고나 할까. 이미 상당히 진행되어 반고의 신격 주변의 생물들은 태고의 본체로 돌아가고 있다네.', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            ::NPC_95_0005::
            btn = me:dialog(npc, '오, 그래보겠는가? 자네는 이미 상당한 수련을 거쳐 인간의 한계에 근접하고 있는 것으로 보이는군. 물론 신에 대항한다는 것이 쉽진 않겠지만, 내 자네를 믿어보도록 하지.', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            ::NPC_95_0006::
            btn = me:dialog(npc, '이 일을 해결한다면 자네 한단계 더 높은 경지에 다다를 수 있을 것이야. 지금부터 내가 하는 말을 끝까지 잘 들어보게.', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            ::NPC_95_0007::
            btn = me:dialog(npc, "반고의 기운이 점점 강해지고 있는 것으로 보아 부활의 날이 다가오고 있지만, 아직은 희망이 있네. 그것은 오제들이 아직 반고 부활의 의식을 완성하지 못했기 때문일세. 즉 반고 부활의 제물을 찾지 못했다는 것일세. 그들이 그 제물을 찾아내기 전에 저지해야만 하네.", { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            ::NPC_95_0008::
            btn = me:dialog(npc, "자네가 해야할 일은 반고의 부활을 꾀하고 있는 오제를 제거하고 증표를 구해오는 것일세. 오제란 소호, 전욱, 제곡, 요제, 순제를 말하지. 먼저 장안성 성곽 부근에 위치한 신전으로 가서, 소호를 물리치고 돌아오게.", { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
        ...
        if step == 2 then
            ...
            q:step(3)
            ::NPC_95_0011::
            btn = me:dialog(npc, '전욱까지 제거하다니, 잘 해주고 있네.\n하지만 아직은 갈 길이 멀다네.\n이번엔 제곡일세. 그의 신전은 부여성에 있다네.', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_95_0012::
        ...
        if step == 6 then
            ...
            q:step(7)
            ::NPC_95_0022::
            btn = me:dialog(npc, '자네를 다시 보게 되어 반가우면서도 놀랍기 그지 없네 그려.\n인간으로서 반고를 제압할 수 있는 자가 있을 줄은 나로선 상상하지도 못했었네.\n이로서, 멸절의 위기를 넘겼으니 기쁘긴 하네만, 우리가 추악한 욕심을 버리고 세상을 스스로 정화하지 못한다면 언젠가 이런 일이 다시 생길 수 있을 것이야.', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_95_0023::
