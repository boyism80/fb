...
function bokgeon_sell_doll(me, npc)
    ...
    if q == nil then
        ...
        ::NPC_235_0008::
        local btn = me:dialog(npc, '앗, 귀한 장난감을 가지고 계시네요. 그건... 청자다람쥐인형이군요.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_235_0009::
        local sel, btn = me:list(npc, '그걸 제게 파시지 않겠어요?', { '그래, 팔께!', '아니, 됐어.' }, { prev = true })
        ...
        ::NPC_235_0010::
        btn = me:dialog(npc, '고맙습니다! 음... 이건 꽤 귀한 장난감이니까 10만전에 살게요!', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
        ...
        btn = me:dialog(npc, '다른 재미있는 장난감이 있으면 제게 가져와 주세요. 제가 다 사드릴게요.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
...
function bokgeon_find_toys(me, npc)
    ...
    if q == nil then
        ::NPC_235_0020::
        local btn = me:dialog(npc, '아... 대체 어디 있지? 어디서 잃어버렸더라? 뒤적뒤적...', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        local sel, btn = me:list(npc, '어, 누구세요? 제게 하실 말씀이라도 있나요?', { '난 그냥 지나가는 사람인데. 뭘 그리 찾고 있니?', '아니, 아무것도 아니야.' }, { prev = true })
        if btn == DIALOG_RESULT.PREV then
            goto NPC_235_0020
        end
        if sel == nil or sel ~= 1 then
        ...
        ::NPC_235_0018::
        btn = me:dialog(npc, '아, 잃어버린 장난감을 찾고 있어요. 여기 복건성에 계시는 관리가 제 아버님이시죠.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_235_0019::
        sel, btn = me:list(npc, '귀한 장난감들인데, 이번에 또 잃어버리면 아버님이 더 이상 사주시지 않겠대요. 이를 어쩌지...', { '장난감은 내가 찾아 줄 테니 여기서 기다리렴.', '그래, 열심히 찾으렴' }, { prev = true })
        if btn == DIALOG_RESULT.PREV then
            goto NPC_235_0018
        end
        if sel == nil or sel ~= 1 then
        ...
        btn = me:dialog(npc, '그럼 저는 여기서 기다릴께요. 장난감을 찾으시거든 제게 말씀해 주세요.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
    ...
    if me:has_items(materials) then
        ::NPC_235_0030::
        local btn = me:dialog(npc, '우와! 상아주사위, 오색폭죽, 청옥팽이를 모두 찾아 주셨군요! 대단해요! 정말 고맙습니다.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        btn = me:dialog(npc, '찾느라 힘드셨죠? 이건 팔과탕이라는 요리인데, 맛도 좋고 약으로도 쓰이는 우리 전통요리랍니다. 한 번 드셔보세요.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        local code = me:exchange(
