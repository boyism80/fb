...
function NPC_236(me, npc)
    ...
    if q == nil then
        ::NPC_236_0001::
        local btn = me:dialog(npc, '요즘 백성들이 후를 두려워해서 집밖으로 다니는것 조차 두려워하고 있다네. 일정한 곳에 머물지 않고 땅을 파고 어디든지 돌아다니기 때문에 더욱 두려운 존재야.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_236_0002::
        btn = me:dialog(npc, '이놈들을 퇴치하지 않으면 머지않아 우리나라가 쑥밭이 될 지경이니 이거 정말 큰일이네.', { prev = true, next = true })
        ...
        local sel = me:list(npc, '자네가 후를 퇴치하여 주지 않겠나? 제발 부탁하네.', { '그러죠. 제가 처리하겠습니다!!', '저보다 나은 적임자가 분명 있을겁니다.' }, { prev = false })
        if sel == nil or sel ~= 1 then
            return
        end
        ::NPC_236_0003::
        btn = me:dialog(npc, '고맙네. 후는 번식력도 강해서 한번에 ' .. BONE_COUNT .. '마리 이상 잡지 않으면 금방 다시 번식을 한다네. 후는 여러종류가 있지만, 어떤 종류든 후를 잡으면 뼈를 남긴다네.', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_236_0004::
    ...
    if q:step() == 1 then
        ...
        ::NPC_236_0005::
        btn = me:dialog(npc, BONE_COUNT .. '마리를 잡는데 성공했군! 하지만 아직 일이 끝난게 아닐세. 미리 얘기를 하지 않았지만 후라는 녀석은 마계천신이라는 왕이 있어서 그 왕이 사라지지 않는 이상 시간이 지나면 더욱 강하게 되살아나지.', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_236_0006::
        ...
        local sel = me:list(npc, '마계천신의뼈를 가져와줄 수 있겠나?', { '물론이죠! 다시는 되살아나지 못하도록 해야합니다.', '그런것까지 제가 할 필요는 없는것 같군요.' }, { prev = false })
        if sel == nil or sel ~= 1 then
            return
        end
        q:step(2)
        ...
        ::NPC_236_0007::
        btn = me:dialog(npc, '오, 고맙네. 그럼 자네가 가지고온 후의뼈는 가져가도록 하겠네.', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_236_0008::
    ...
    if q:step() == 2 then
        ...
        ::NPC_236_0009::
        btn = me:dialog(npc, '오오, 정말로 마계천신의뼈를 가지고 왔군. 자넨 정말 대단한 인물이군..', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_236_0010::
        btn = me:dialog(npc, '정말 진심으로 고맙네. 자넨 정말 큰일을 해냈어! 허허...', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then
            goto NPC_236_0009
        end
        if btn == DIALOG_RESULT.QUIT then
        ...
        ::NPC_236_0011::
        btn = me:dialog(npc, '이건 고마움의 표시니 가져가도록 하게. 고구려에 있다는 숨겨진 마을에서 만들었다는 \'세시투구\'를 흉내내어 만들어 본 것이라네. 물론 세시투구만큼의 성능이 나오진 않지만 말이야.', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then
            goto NPC_236_0010
        end
        if btn == DIALOG_RESULT.QUIT then
