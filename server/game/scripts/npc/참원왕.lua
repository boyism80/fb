...
function NPC_474(me, npc)
    ...
    if main_step == 1 then
        ...
        b = me:dialog(npc, "나에게 알려주지 않겠나? 부탁하겠네. 아마, 모든 왕들을 한번씩은 만나봐야 할게야. 여유있게 다녀오시게.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
        main:step(2)
    ...
    if main_step == 5 then
        ...
        b = me:dialog(npc, "어쩔 수 없지. 자, 이 편지를 가져다가 보여주시게. 그래도 아니 될 일이라면, 다른 대책을 강구해보는 수 밖에 없겠지.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
        me:mkitem("원숭이의편지", 1)
    ...
    if main_step == 7 then
        ...
        b = me:dialog(npc, "자, 그럼 다른 왕들의 계책도 들어다주게나. 잘 부탁하네.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
        main:step(8)
    ...
    if main_step == 9 then
        local b = me:dialog(npc, "그래, 역시 문자를 알아야 연구를 할 수 있겠지. 음, 혹시 이 나라 사람들이 배우는 글책을 구할 수 있을까?", true, true)
        if b == DIALOG_RESULT.QUIT then
            return
        end
    ...
    if main_step == 11 then
        ...
        b = me:dialog(npc, "나머지 한 권은 대원왕에게 가져다 주게. 필요한 재료를 만드는데 도움이 될 걸세.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
    ...
    if main_step == 14 then
        ...
        b = me:dialog(npc, "아, 고맙네. 귀중한 목숨이 위험할때 반드시 요긴하게 쓰도록 하겠네. 자네는 정말 대단한 사람이구만...", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
    ...
    if main_step == 15 then
        ...
        b = me:dialog(npc, "좀 더 수고해서 다른 왕들의 의견을 들어줬으면 하네.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
        main:step(16)
    ...
    if main_step == 17 then
        ...
        b = me:dialog(npc, "틀린 말은 아닐세. 철도 3개와 사각방패 3개 정도면 일단 작은 자경단 정도는 구성할 수 있을게야.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
        main:step(18)
    ...
    if main_step == 19 then
        ...
        b = me:dialog(npc, "그럼, 다른 두목들의 계책과 의견을 마저 모아다 주시게나. 기다리고 있겠네.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
        main:step(20)
    ...
    if main_step == 21 then
        ...
        b = me:dialog(npc, "일단 횃불이란걸 나에게 가져다주게. 몸소 실험해보고, 괜찮다 싶으면 돌려 써야하겠어.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
        main:step(22)
    ...
    if main_step == 22 then
        ...
        b = me:dialog(npc, "더이상 무슨 의견이나 계책이 나올지 의심스럽지만, 마지막으로 한 번만 더 수고해주지 않겠나.", true, true)
        return
    end
    ...
    if main_step == 24 then
        ...
        b = me:dialog(npc, "아이들이라도 편하게 잘 수 있다면 한시름 놓을 수 있을게야... 잘 부탁하네.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
        main:step(25)
    ...
    if main_step == 35 then
        ...
        b = me:dialog(npc, "원숭이들의 왕인 나 참원왕의 가족일세. 자, 이걸 받으시게. 우리 참원왕 대대로 내려오는 해골 목걸이야.", true, true)
        if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
            return
        end
        main:step(36)
