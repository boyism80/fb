...
function NPC_214(me, npc)
    ::NPC_214_COS001::
    local sel = me:list(npc, '아함~ 졸려.... 무슨일로 왔는가? 난 바쁜사람이라네...', { '보물열쇠를 가지고 왔어요...', '하하.. 그냥 인사차...' }, false)
    if sel == nil then
        return
    end
    ...
    local btn = me:dialog(npc, '오호 그런가? 보물열쇠를 가지고 오면 내가 바꿔주긴 하는데...', false, false)
    ...
    local key_opt = me:list(npc, '그래... 무슨 열쇠를 가지고 왔나?', key_options, false)
    if key_opt == nil then
        return
    end
