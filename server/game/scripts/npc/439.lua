...
local function do_craft(me, npc)
    ...
    if code == enum.exchange_result.LACK_COST then
        me:dialog(npc, "눈꽃얼음이 없으신데요?", { prev = false, next = false })
        return false
    end
    ...
    if reward == nil then
        me:dialog(npc, "이런... 그만 실패하고 말았군요. 다음 기회를 기약하셔야 할 듯 싶네요...", { prev = false, next = false })
    else
        me:dialog(npc, "오... " .. REWARD_NAME .. "을 만드는데에 성공했습니다!! 잘 사용하시길 바라겠습니다.", { prev = false, next = false })
    end
    return true
end
...
return {
    on_click = function(me, npc)
        ...
        d = me:dialog(npc, "눈꽃얼음을 10개 가져오시면, 제 나름대로 심혈을 기울여 " .. REWARD_NAME .. "을 만들어 볼 수 있습니다.", { prev = false, next = true })
        if d == DIALOG_RESULT.QUIT then
            return
        end
        d = me:dialog(npc, "눈꽃얼음 10개를 가져오시면 [" .. REWARD_NAME .. "]을 만들어 드릴 수 있습니다. 하지만 성공은 장담하지 못하는데... 괜찮으시겠습니까?", { prev = false, next = true })
        if d == DIALOG_RESULT.QUIT then
            return
        end
        ...
        d = me:dialog(npc, "자... 그럼 눈꽃얼음을 이용해 한번 " .. REWARD_NAME .. "을 만들어 보도록 하겠습니다.", { prev = false, next = true })
        if d == DIALOG_RESULT.QUIT then
            return
        end
