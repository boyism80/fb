...
function NPC_439(me, npc)
    ...
    d = me:dialog(npc, "눈꽃얼음을 10개 가져오시면, 제 나름대로 심혈을 기울여 " .. REWARD_NAME .. "을 만들어 볼 수 있습니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
    d = me:dialog(npc, "눈꽃얼음 10개를 가져오시면 [" .. REWARD_NAME .. "]을 만들어 드릴 수 있습니다. 하지만 성공은 장담하지 못하는데... 괜찮으시겠습니까?", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
