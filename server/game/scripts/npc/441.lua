...
local function run_snow_helmet_info(me, npc)
    ...
    d = me:dialog(npc, "눈꽃투구는 매우 훌륭한 방어구임에 틀림 없지만, 그 특성상 함부로 사용하기가 어렵다는 단점이 있습니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
    d = me:dialog(npc, "갓 제작한 눈꽃투구는 그 한기가 너무 강력해, 착용자의 체력과 마력을 매우 많이 앗아간다는 치명적인 결함을 가지고 있지요.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
    d = me:dialog(npc, "하지만, 가공하기에 따라서는 그 결함 또한 최소화 할 수 있습니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end

    local sel, btn = me:list(npc, "궁금한점은 해결이 되셨는지요?", { "눈꽃투구는 어떻게 가공할 수 있나요?", "알려주셔서 감사합니다." }, false)
    ...
    d = me:dialog(npc, "아시다시피 눈꽃투구를 만들기 위해서는, 눈꽃얼음이 필요합니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
    d = me:dialog(npc, "이 눈꽃얼음은 압축하면 할 수록 단단해지고, 내부의 냉기가 밖으로 새어나오지 않는 특이한 성질을 가지고 있지요...", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
    d = me:dialog(npc, "눈꽃투구의 결함은 총 열단계를 거쳐 개선될 수 있습니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
    d = me:dialog(npc, "첫번째 단계는 눈꽃얼음 10개, 두번째 단계에는 눈꽃얼음 20개, 그 다음 단계에는 30개가 필요하고, 그렇게 마지막 열단계 째에는 100개의 눈꽃얼음이 필요합니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end

    sel, btn = me:list(npc, "만약 눈꽃투구와, 눈꽃얼음을 가져오셨다면 더 단단하게 압축하여 결함률을 줄여드리겠습니다.", { "예. 부탁드리겠습니다.", "아니오... 다음에..." }, false)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if sel == 1 then
