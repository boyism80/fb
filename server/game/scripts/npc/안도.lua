function NPC_181(me, npc)
    ...
    if quest == nil then
        ::NPC_181_COS001::
        btn = me:dialog(npc, '혹시 자네도 이 이글거리는 기운이 보이는가? 집 전체에 귀기가 서려서 잠을 잘 수가 없군. 집값이 싸서 샀더니만 이런 귀신 붙은 집일 줄 몰랐어.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_181_COS002::
        btn = me:dialog(npc, '지나가던 스님께 여쭤 봤더니 부적을 방 구석구석 바닥에 묻으면 귀신을 퇴치할 수 있다고 하더군.', true, true)
        ...
        ::NPC_181_COS004::
        btn = me:dialog(npc, '그래 주겠는가? 고맙네.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        btn = me:dialog(npc, '이 부적을 가지고 집 구석구석을 살피다가... 허술해 보이는 부분이 있으면 바닥에 묻어주게나. 잘 부탁하네.', true, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_181_COS004
        end
        if btn == DIALOG_RESULT.QUIT then
    ...
    if quest:step() == 1 then
        ...
        ::NPC_181_COS006::
        btn = me:dialog(npc, '부적이 다 떨어졌다고? 큰일이군! 부적이 더 있어야겠는데...', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_181_COS007::
        btn = me:dialog(npc, '그런데 이 부적은 예전에 중국에 갔을 때 귀신 쫓는 데에 능하다는 고승에게 받은 것이라 더 받으려면 중국까지 가야 할 판국이야. 이거 야단났군...', true, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_181_COS006
        end
        if btn == DIALOG_RESULT.QUIT then
        ...
        ::NPC_181_COS009::
        btn = me:dialog(npc, '정말 고맙네! 자넨 혹시 보살인가? 정말 친절하구만. 일이 끝나고 나면 내 꼭 사례하겠네.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        btn = me:dialog(npc, '그렇지. 그 스님을 찾는다면 이름이라도 가르쳐 줘야겠군. 뭐라고 했더라? ...아, 그렇지.\n\n중국에 있다는 그 시님의 법명은 \'마타\'라고 했다네. 미안하지만 부적을 얻어 와 주게나. 부탁하네.', true, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_181_COS009
        end
        if btn == DIALOG_RESULT.QUIT then
    ...
    if quest:step() == 3 then
        ...
        ::NPC_181_COS011::
        btn = me:dialog(npc, '다 끝났군, 이젠 좀 살만하구만!', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_181_COS012::
        btn = me:dialog(npc, '다 자네 덕분일세, 약속대로 사례를 해야겠는데... 뭐가 좋을까... 그렇지. 이걸 가져가게.', true, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_181_COS011
        end
        if btn == DIALOG_RESULT.QUIT then
        ...
        ::NPC_181_COS013::
        btn = me:dialog(npc, '이건 내가 예전에 귀신을 쫓아 보려고 샀던 부적이야. 비슷하게 생겨서 효과가 있으려니 하고 샀더니만 귀신 쫓는 데에는 효과가 없고 그저 머리가 좀 맑아지더구만.', true, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_181_COS012
        end
        if btn == DIALOG_RESULT.QUIT then
        ...
        btn = me:dialog(npc, '나처럼 머리 나쁜 사람한테는 별 쓸모가 없겠지만 자네한테는 쓸모가 있을 것 같으니 가져가주게나.', true, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_181_COS013
        end
        if btn == DIALOG_RESULT.QUIT then
