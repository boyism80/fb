...
function NPC_108(me, npc)
    ...
    if q:step() == 2 then
        ...
        local sel = me:list(npc, me:name() .. '씨가 하나 구해주시지 않겠소?', { '제가 구해드리겠습니다.', '저는 좀 바빠서..' }, false)
        if sel == nil then
            return
        end
        if sel == 1 then
    ...
    if q:step() == 3 then
        ...
        local sel = me:list(npc, me:name() .. '씨, 인어의거울도 좀 구해다 주실수 없겠소?', { '네, 구하는 김에 거울도 구하지요.', '갑자기 바쁜 일이 생겨서...' }, false)
        if sel == nil then
            return
        end
        if sel == 1 then
    ...
    if q:step() == 4 then
        ...
        ::NPC_108_0031::
        btn = me:dialog(npc, '이걸 어떻게 사례하나.. 그렇지. 우리 집안에 내려오는 목걸이를 하나 드리지.\n\n\'봉마의목걸이\'라고 해서.. 마귀를 봉인하여 담았다고 하는 옥으로 만든것이란느데.. 정말인지는 모르겠소.', false, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_108_0030
        end
        if btn == DIALOG_RESULT.QUIT then
