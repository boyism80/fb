function NPC_236(me, npc)
    ...
    if quest == nil then
        ::NPC_236_COS001::
        local btn = me:dialog(npc, '요즘 백성들이 후를 두려워해서 집밖으로 다니는것 조차 두려워하고 있다네. 일정한 곳에 머물지 않고 땅을 파고 어디든지 돌아다니기 때문에 더욱 두려운 존재야.', true, true)
        ...
        local sel = me:list(npc, '자네가 후를 퇴치하여 주지 않겠나? 제발 부탁하네.', { '그러죠. 제가 처리하겠습니다!!', '저보다 나은 적임자가 분명 있을겁니다.' })
        ...
        btn = me:dialog(npc, '자네가 후를 ' .. BONE_COUNT .. '마리 이상 잡았다는 증거로, ' .. BONE_COUNT .. '마리의 뼈를 가지고 오면 내 포상을 하지. \'후의뼈\' ' .. BONE_COUNT .. '개라네.', true, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_236_COS002
        end
        if btn == DIALOG_RESULT.QUIT then
    ...
    if quest:step() == 1 then
        ...
        btn = me:dialog(npc, '그래서 한가지 부탁을 더 해야겠네. 마계천신을 잡아 그의 뼈를 가져와주게. 정말 마지막 부탁이네.', true, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_236_COS003
        end
        if btn == DIALOG_RESULT.QUIT then
        ...
        local sel = me:list(npc, '마계천신의뼈를 가져와줄 수 있겠나?', { '물론이죠! 다시는 되살아나지 못하도록 해야합니다.', '그런것까지 제가 할 필요는 없는것 같군요.' })
        if sel == nil or sel ~= 0 then
            return
        end
        quest:step(2)
    ...
    if quest:step() == 2 then
        ...
        btn = me:dialog(npc, '이 투구를 쓰고 있으면 자네의 무기와 갑옷의 내구력이 오랫동안 유지될걸세. 그럼 잘 가시게..', true, true)
        if btn == DIALOG_RESULT.PREV then
            goto NPC_236_COS007
        end
        if btn == DIALOG_RESULT.QUIT then
