function NPC_175(me, npc)
    ...
    if quest == nil then
        ::NPC_175_COS001::
        btn = me:dialog(npc, '난 고구려의 무도가요. 얼마전부터 중국의 소림사에 대해 관심을 가지게 됐소. 정말 아무도 그 위치나 규모에 대해서 아는 사람이 없었소.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_175_COS002::
        btn = me:dialog(npc, '내 얼마전에 듣기로는 만리장성 부근에 출중한 무술 기량을 가진 사람들이 보인다 하던데. 그들은 모두 승려차림이었다하오.\n\n그들의 절기를 꼭 한번 실제로 보고 배우고싶은데 마땅한 기회가 없구려.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
    ...
    if quest:step() < 4 then
        ...
        if not has_baekyeoljang then
            me:dialog(npc, '아직 소림사의 절기를 배워오지 못하신 것 같군요.', false, true)
            return
        end
        ::NPC_175_COS004::
        btn = me:dialog(npc, '그래.. 소림사의 절기는 배워오셨소? 오오.. 그 절기의 이름을 백열장이라고 하는군..', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_175_COS005::
        btn = me:dialog(npc, '음.. 그렇군. 대충 기술은 이해했소. 나도 무도가라 한번 보면 대략적인 형태는 이해할수 있다오. 이후엔 수련을 해서 더 높은 경지로 갈 수 있도록 해야겠지.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
    ...
    me:dialog(npc, '정말 고맙소. 당신 덕분에 나도 소림사의 절기를 더욱더 배워지고 싶구만.', true, true)
end