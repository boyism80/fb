...
function NPC_211(me, npc)
    ...
    if q == nil then
        ...
        btn = me:dialog(npc, '우선 산소들을 상대하실 수 있는지 봐야겠습니다.', false, true)
    ...
    if step >= 1 and step <= 5 then
        ...
        btn = me:dialog(npc, '갈수록 보상금이 높아지니 부탁드립니다.', false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        return
    ...
    if step == 7 then
        if not me:has_items('산소괴왕의뿔', 1) then
            me:dialog(npc, '아직 산소괴왕의뿔을 구하시지 못하셨군요. 부디 조심하시길 바랍니다.', false, true)
            return
        end
        btn = me:dialog(npc, '정말 대단하군요. 설마 정말 산소괴왕을 잡아오시리라고는. 이로써 큰 걱정을 덜게 되었군요.', false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ...
        btn = me:dialog(npc, '부디 좋은 곳에 써주시기 발바니다. 그럼 안녕히.', false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        return
