...
return {
    ON_CLICK = function(me, npc)
        ...
        if step == 10 then
            local btn = me:dialog(npc, "해골왕의 증표인 [해골왕의뼈], 그리고 유성지의 증표인 [유성지의보패], 그리고 내 기운을 담아 낼 금속인 [철광석], 마지막으로 이를 한데 섞기 위한 [불의수정]이 필요하다네.", { prev = false, next = true })
        ...
        for i = 1, #REQUIRED_ITEMS do
            ...
            if not me:has_items(name, count) then
                me:dialog(npc, name .. "가 없군...", { prev = false, next = false })
                return
            end
        end
        ...
        for i = 1, #REQUIRED_ITEMS do
            ...
            if not me:rmitem(name, count, ITEM_DELETE_TYPE.GIVE) then
                me:dialog(npc, name .. "를 건네지 못했습니다. 인벤을 확인해 주세요.", { prev = false, next = false })
                return
            end
        end
        ...
        me:dialog(npc, "증표를 잘 받았다. 의식은 이곳의 기운이 맞을 때 진행되지.", { prev = false, next = false })
        npc:destroy()
