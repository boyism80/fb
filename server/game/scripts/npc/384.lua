...
function NPC_384(me, npc)
    ...
    ::NPC_384_0002::
    btn = me:dialog(npc, "우리 소림사에서 알려준 비기를 그렇게 함부로 대하다니... 흥!! 자네는 어찌 된 것이 그 모양인가? ", true, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    ...
    ::NPC_384_0003::
    btn = me:dialog(npc, "그래... 어디 반성하고 있길 바라겠네. 그런데...뭐라고? 다시 배우고 싶다니 지금 나와 장난하자는 겐가? 혜천대사님이 화가날 모습을 생각하니 으으...", true, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if btn == DIALOG_RESULT.PREV then
    ...
    ::NPC_384_0004::
    btn = me:dialog(npc, "알았네. 그럼 내 혜천대사님께 잘 일러둘테니, 자네의 그 무지함을 반성하는 의미에서 소림사에 시주를 받아야겠네.", true, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if btn == DIALOG_RESULT.PREV then
