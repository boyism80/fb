...
return {
    on_click = function(me, npc)
        ...
        if sel == 1 or sel == 2 then
            me:dialog(npc, "이미 참여 하셨네요. 한 계정에 한번만 참여가 가능합니다. (참여 기능은 준비중입니다.)", { prev = false, next = false })
        elseif sel == 4 or sel == 5 or sel == 6 or sel == 7 then
            me:dialog(npc, "준비중입니다.", { prev = false, next = false })
        elseif sel == 8 then
            me:dialog(npc, "당첨여부 확인은 준비중입니다.", { prev = false, next = false })
        else
            me:dialog(npc, "준비중입니다.", { prev = false, next = false })
        end
    end
