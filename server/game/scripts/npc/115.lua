...
local function cultural_property_quest(me, npc)
    ...
    if q:completed() then
        me:dialog(npc, '일은 정말 힘들군!', { prev = false, next = false })
        return
    end
    ...
    if selected == 2 then
        ...
        if next_tier == nil then
            me:dialog(npc, '아직 한 일이 별로 없는거 같은데? 토템을 적어도 5번은 가져와야 보상을 줄 수 있네.', { prev = false, next = false })
            return
        end
        local reward_name = reward_items[next_tier]
        if me:mkitem(reward_name, 1) == nil then
            me:dialog(npc, '소지품이 가득 차서 ' .. name_with(reward_name, '을', '를') .. ' 줄 수 없네.', { prev = false, next = false })
            return
        end
        q:step(next_tier)
        if next_tier == 3 then
