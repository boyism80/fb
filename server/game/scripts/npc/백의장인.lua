...
local function run_dragon_weapon_awaken(me, npc)
    ...
    local success_item = line.success[grade_sel + 1]
    local failed_item = line.failed[grade_sel + 1]
    local rate = DRAGON_AWAKEN_RATES[grade_sel + 1]
...
local function run_amber_weapon_craft(me, npc, colors)
    ...
    local color_name = colors[color_sel + 1].color
    local weapon_opts = {}
    ...
    if weapon_sel == nil or weapon_sel < 0 or weapon_sel >= #WEAPON_NAMES then
        return DIALOG_RESULT.QUIT
    end
