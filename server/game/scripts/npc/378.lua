...
return {
    on_click = function(me, npc)
        ...
        if param == "1" then
            ...
            if sel == 1 then
                local btn = me:dialog(npc, "뭐라구! 지금 제정신으로 하는 소리인가! 내가 여기 상거래 허가를 얻기위해 도대체 얼마나 많은 뇌물을 뿌렸는지 모르는군!", { prev = false, next = true })
                if btn == DIALOG_RESULT.QUIT then
                    return
                end
                local btn = me:dialog(npc, "음? 아니아니, 방금 그 말은 잊어주게. 어흠어흠흠... 나는 한 사람의 자랑스러운 상인으로서 좋은 물건은 세상 모든 사람들이 사서 쓸 수 있도록 공급해야한다는 나의 철학을 지켜야만 하네. 따라서 여기에서 포기할 순 없지!", { prev = false, next = true })
                if btn == DIALOG_RESULT.QUIT then
                    return
                end
            ...
            if sel == 3 then
                ...
                if sel2 == 1 then
                    local btn = me:dialog(npc, "뭐? 뭐라구! 괴물 퇴치 비용을 달라구! 아니, 이런 치사스런 사람을 봤나! 너무한거 아닌가! 남의 불행을 이용하다니!\n\n이런 천인공노할 사람을 봤나! 세상에나! 세상에나! 아이고, 억울해! 억울해서 배가 다 아프고, 머리까지 아프네!", { prev = false, next = true })
                    if btn == DIALOG_RESULT.QUIT then
                        return
                    end
        ...
        if sel == 1 then
            local btn = me:dialog(npc, "오, 그래? 필요없는가 본데, 내가 사줄 수도 있지. 이야, 이미 사람 손탄거라 비싸고 주고 싶어도 비싸게는 못줘.", { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            local btn = me:dialog(npc, "아니, 뭐. 아직 보급형이 나오지 않아서 등록비도 만만찮고 말이야. 사실 말이야 바른 말이지. 이런 물건은 높으신 분이나 그 자제분들이나 가지고 노는 물품이라구. 어떤가? 지금이라면 내가 인심 팍팍 써서 30만전에 사주지.", { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
