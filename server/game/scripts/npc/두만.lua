...
function NPC_379(me, npc)
    ...
    if quest == nil then
        ...
        btn = me:dialog(npc, "보아하니 행동력과 뛰어난 두뇌를 지닌 것 같은데, 임시 수사관으로써 한 번 범인을 밝혀내보지 않겠나?", true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
        ...
        me:dialog(npc, "그리고 나와 동생들이라고 놀고 있지는 않을테니까 가끔 들러서 상황도 알려주시게. 우리도 얻은 정보를 그때그때 알려줄테니까.", true, false)
        return
    end
    ...
    if sel == 0 then
        if step == 1 then
            ...
            btn = me:dialog(npc, "아무리 부자라고 해도 요즘들어 재산이 몇 배씩 불어나고 있었거든. 이유를 몰라서 어디서 금이라도 캐오나 싶었더니만.", true, true)
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            me:dialog(npc, "자네 말대로 이제는 그 뚱보 부자들이 제일 의심스럽네. 자네의 그 추리력을 발휘해서 진상을 밝혀주게나!", true, false)
        elseif step == 4 then
            ...
            me:dialog(npc, "난생처음 들어보네! 세상에! 아..아이구! 혈..혈압이 머리 꼭대끼까지 솟아오르네...아이구 아이구;", true, false)
            if quest then
                quest:step(5)
            end
        else
    ...
    if sel == 1 then
        if step == 10 then
            ...
            btn = me:dialog(npc, "그래, 자네 말대로 무언가 팔 물건도 가져왔겠지... 근데 우리 섬에서 외부 물건을 탐낼만한 사람이", true, true)
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            me:dialog(npc, "부자들말고 따로 있을까? 하긴, 부자들만 외부 세상 물건에 관심 있으란 법은 없긴하지.", true, false)
            if quest then
                quest:step(11)
            end
        elseif step == 15 then
            ...
            me:dialog(npc, "감사의 뜻으로 10만전을 드리지! 정말 수고했네! 큰일을 해냈어! 하하하하하하하!", true, false)
            if quest then
                quest:step(16)
                quest:complete()
            end
