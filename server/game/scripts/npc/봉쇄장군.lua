...
function NPC_473(me, npc)
    ...
    button = me:dialog(npc, "병사들의 이야기를 들어보면 알겠지만, 지금 원숭이들은 먹이를 잘못 먹고는 흉폭해지는 병에 걸려있다네.", false, true)
    if button == DIALOG_RESULT.QUIT then
        return
    end

    button = me:dialog(npc, "다행히 전염병이 아니어서 아직 정상인 원숭이들도 많지만, 흉폭해진 원숭이들에 비하면 형평없이 약해서 도망다닐 뿐 이라네.", false, true)
    if button == DIALOG_RESULT.QUIT then
        return
    end

    button = me:dialog(npc, "흉폭해진 원숭이들은 치료를 해줄 수도 없어. 다만 처단하는 것만이 해결책이야. 들어가서 용감히 싸워주면 고맙겠군.", false, true)
    if button == DIALOG_RESULT.QUIT then
        return
    end

    button = me:dialog(npc, "다만 조심하시게 원숭이들은 머리도 좋고, 힘도 굉장히 강하니까 말이야. 일국의 장군인 나도 몇 번이나 당할 뻔 했지.", false, true)
    if button == DIALOG_RESULT.QUIT then
        return
    end
