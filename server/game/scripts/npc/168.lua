...
return {
    on_click = function(me, npc)
        ...
        if q == nil then
            ...
            ::NPC_168_0010::
            btn = me:dialog(npc, '환갑잔치에 쓸 요리가 필요한데, 변변한 요리사가 없어서 말이야. 환갑잔치는 평생 한번 있는 경사인데, 그런 날에 맛없는 요리를 드셔서야 안 되지.', { prev = true, next = true })
            ...
            local sel, lb = me:list(npc, '아버님께 맛있는 것을 많이 드시게 하고 싶은데, 요리사를 구할 수가 없으니 참 큰일이야..', { '저도 요리를 만들 줄 아는데, 도와드릴까요?', '찬찬에게 맡겨보시지 그러세요?' }, { prev = false })
            if lb == DIALOG_RESULT.QUIT then
                return
            end
            if sel == nil or sel == 2 then
                if sel == 2 then
                    me:dialog(npc, '그렇다면 좋겠지만 찬찬은 너무 멀리 있어서 말이지...', { prev = false, next = false })
                end
                return
            end
            if sel ~= 1 then
        ...
        if sel == 2 then
            me:dialog(npc, '흠.. 곧 환갑잔치가 시작하니 조금만 서둘러 주게.', { prev = false, next = false })
            return
        end
        if sel ~= 1 then
            return
        ...
        ::NPC_168_0051::
        btn = me:dialog(npc, '그런가! 수고했네. 그럼 어떤 요리를 가져왔는지 꺼내 보게나.', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
