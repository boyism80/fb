...
function NPC_621(me, npc_obj)
    local sel, list_btn = me:list(npc_obj, "안녕하세요. 무엇을 도와드릴까요?", {
        "물건사기",
        "물건팔기",
        "제작 의뢰",
    }, false)
    ...
    local craft_sel, craft_btn = me:list(npc_obj, "안녕하세요. 어떤것을 만들어 드릴까요?", {
        "원앙의청",
        "원앙의적",
        "표범가죽옷",
        "곰가죽옷",
    ...
    local function do_craft(need_items, money_needed, product_name, rate_percent)
        ...
        local btn = me:dialog(npc_obj, req_msg, false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_621_0002::
        btn = me:dialog(npc_obj, "실패하더라도 저를 너무 원망하지는 말아주세요...", true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if code == server.EXCHANGE_RESULT.LACK_COST then
            for item_name, need_count in pairs(need_items) do
                if not me:has_items(item_name, need_count) then
                    me:dialog(npc_obj, name_with(item_name, '이', '가') .. " 부족합니다.", false, false)
                    return
                end
            end
            me:dialog(npc_obj, "금전이 부족합니다.", false, false)
            return
        end
        if code == server.EXCHANGE_RESULT.LACK_CAPACITY then
            me:dialog(npc_obj, "소지품이 가득 차서 줄 수 없습니다.", false, false)
            return
        end
        if reward == nil then
            me:dialog(npc_obj, "아앗... 깃털이 그만 접착제와 덕지덕지 붙어버려서 실패하고 말았어요... 죄송합니다....", false, false)
        else
            me:dialog(npc_obj, "자, 여기 [" .. product_name .. "] " .. name_with(product_name, '을', '를') .. " 만들어 드렸습니다. 예쁘게 입고 다니세요.", false, false)
        end
    end
    ...
    elseif craft_sel == 2 then
        ...
        ::NPC_621_0003::
        local btn = me:dialog(npc_obj, "[" .. product .. "] " .. name_with(product, '을', '를') .. " 만들기 위해서는, [담비가죽] 100개, [흑담비가죽] 100개, [표범의가죽] 100개 그리고 금전 250만전 을 주셔야 합니다.", false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_621_0004::
        btn = me:dialog(npc_obj, "성공률은 3할로, 실패하더라도 저를 너무 원망하지는 말아주세요...", true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
    ...
    elseif craft_sel == 3 then
        ...
        ::NPC_621_0005::
        local btn = me:dialog(npc_obj, "[" .. product .. "] " .. name_with(product, '을', '를') .. " 만들기 위해서는, [담비가죽] 100개, [흑담비가죽] 30개, [백두곰가죽] 100개 그리고 금전 100만전 을 주셔야 합니다.", false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ::NPC_621_0006::
