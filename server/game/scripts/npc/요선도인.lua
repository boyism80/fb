...
function NPC_622(me, npc)
    ...
    button = me:dialog(npc, "하지만 최근들어 정령들이 암흑의 기운으로 폭주하고 난 뒤로... 사람들을 가차없이 공격하기에, 나는 그들이 기거하는 동굴의 앞에서 이렇게 정령들을 관찰하고 있소.", true, true)
    if button == DIALOG_RESULT.QUIT then
        return
    end
    button = me:dialog(npc, "요일의 정령들의 기운이 담긴 돌을 백개씩 가져오면, 내가 할 수 있는 것을 해 주겠소...", true, true)
    if button == DIALOG_RESULT.QUIT then
        return
    end
    ...
    if sel >= 0 and sel <= 6 then
        ...
        local sub_sel, sub_btn = me:list(npc, "[" .. r.product .. "]를 만들기 위해서는 [" .. r.main .. "] 100개와, 다른 지석 100개가 필요하오. 무엇을 주시겠소?", STONES, true)
        ...
        d = me:dialog(npc, "그렇군... [" .. r.main .. "]과 [" .. other .. "]으로 [" .. r.product .. "]를 만들어 보겠소. 다만 성공 확률은 삼할이 채 되지 않으니 실패하더라도 양해해 주시오...", false, true)
        if d == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if reward == nil then
            me:dialog(npc, "이런... [" .. r.product .. "]를 만드는데 실패하고 말았소. 미안하게 됐소...", false, false)
        else
            me:dialog(npc, "[" .. r.product .. "]를 성공적으로 만들었소. 한번 살펴 보시게나....", false, false)
        end
        return
    end
    ...
    if sel == 7 then
        ...
        local sub_sel, sub_btn = me:list(npc, "[칠요구륜]을 만들기 위해서는 모든 요일의 지석 100개가 필요하오. 성공률은 채 일할이 되지 않으니 실패하더라도 양해해 주시오.", { "만들겠습니다.", "아니오... 다시 생각해볼게요." }, true)
        if sub_btn == DIALOG_RESULT.QUIT or sub_sel == nil then
            return
        end
        if sub_sel == 1 then
        ...
        if reward == nil then
            me:dialog(npc, "이런... [칠요구륜]을 만드는데 실패하고 말았소. 미안하게 됐소...", false, false)
        else
            me:dialog(npc, "[칠요구륜]을 성공적으로 만들었소. 한번 살펴 보시게나....", false, false)
        end
        return
    end
    ...
    if sel == 8 then
        ...
        local sub_sel, sub_btn = me:list(npc, "[팔세지도]를 만들기 위해서는 모든 요일의 지석 100개와 팔요지석이 필요하오. 성공률은 채 일할이 되지 않으니 실패하더라도 양해해 주시오.", { "만들겠습니다.", "아니오... 다시 생각해볼게요." }, true)
        if sub_btn == DIALOG_RESULT.QUIT or sub_sel == nil then
            return
        end
        if sub_sel == 1 then
        ...
        if reward == nil then
            me:dialog(npc, "이런... [팔세지도]를 만드는데 실패하고 말았소. 미안하게 됐소...", false, false)
        else
            me:dialog(npc, "[팔세지도]을 성공적으로 만들었소. 한번 살펴 보시게나....", false, false)
        end
        return
    end
    ...
    if sel == 9 then
        ...
        d = me:dialog(npc, "팔요천의 옷을 만들기 위해서는 [일광방패], [월광방패], [화분천검], [수류곡도], [목근정곤], [금기련봉], [토선투구], [칠요구륜], [팔세지도] 모두를 모두 나에게 재물로 가져와야 하오.", false, true)
        if d == DIALOG_RESULT.QUIT then
            return
        end
        d = me:dialog(npc, "또한 현재 성별이 [남성]일 경우 [팔요천의기]를 만들게 되며, 성별이 [여성]일 경우 [팔요천의향]을 만들게 된다오. 이는 음양의 기운에 따른 것이니 바꿀 수 없소.", false, true)
        if d == DIALOG_RESULT.QUIT then
            return
        end
        d = me:dialog(npc, ".....그러나 성공률은 일할이 채 되지 않기 때문에, 만들지 않는 것을 추천하오.", false, true)
        if d == DIALOG_RESULT.QUIT then
            return
        end
        d = me:dialog(npc, "다만 맡겨준다면 최대한 심혈을 기울여 보겠소.  또한 중간에 허튼짓을 하면 모든 아이템이 날아갈 수 있으므로, 집중하도록 하시오.", false, true)
        if d == DIALOG_RESULT.QUIT then
            return
        end
        local sub_sel, sub_btn = me:list(npc, "팔요천의 옷을 만들기 위한 준비가 되었다면, 말을 걸어주시오.", { "만들겠습니다.", "아니오... 다시 생각해볼게요." }, true)
        if sub_btn == DIALOG_RESULT.QUIT or sub_sel == nil then
            return
        end
        if sub_sel == 1 then
        ...
        if reward == nil then
            me:dialog(npc, "아... 이럴수가... 안타깝게 되었소... 그만 실패하고 말았다오...", false, false)
        else
            me:dialog(npc, "오...! 성공하였군. 당신은 정말 하늘의 축복이 함께하는 사람인가 보오. 정말 축하하오!", false, false)
        end
        return
    end
end
