...
function NPC_378(me, npc)
    ...
    if param == "0" then
        if me:level() < 35 then
            local btn = me:dialog(npc, "아이구, 이런... 모처럼 새로운 시장을 제일 먼저 개척하나 싶었더니만 괴물들이 나타나 버리다니.", false, true)
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            btn = me:dialog(npc, "이를 어쩌면 좋지? 이건 죽여도 죽여도 끝이 없이 쏟아져나오니 장수를 할 수가 있나.", false, true)
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            me:dialog(npc, "아휴, 참... 신용만점, 정직만점을 자랑하는 내 인생이 큰오점을 남기게 생겼네...아이구...\n\n(어쩐지 못미덥고 거짓말쟁이같은 상인의 푸념은 끝이 없다.)", false, false)
            return
        end
        local btn = me:dialog(npc, "음? 요술놀이 구슬 특가 판매전? 아, 물론 하려고 했지. 그래, 하려고 했어\n그런데 건물이 다 세워지고 짐을 옮겨놓자마자 괴물들이 들이닥쳤지 뭔가? 북새통에 다 흩어지고 그나마 건진건 목숨밖에 없어.", false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        btn = me:dialog(npc, "그래, 마침 좋은 기회군. 자네, 안에 들어가서 괴물들 좀 해치워주지 않겠나?\n대신 괴물들이 갖고 있는 요술놀이 구슬 부속품들을 가져오면, 내가 저렴하게 조립해주지", false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        btn = me:dialog(npc, "요술구슬, 좌우막대, 상하막대, 누름막대...이렇게 찾아오면 된다네. 되도록 괴물들을 많이 해치워주면 좋겠군.\n\n자, 그럼 부탁하네.", false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if quest == nil then
    ...
    if sel == 0 then
        ...
        if sel2 == 0 then
            ...
            me:dialog(npc, "아주 현명한 선택이야. 자, 15만전 여기있네. 자네 정말 득본거야. 흐흐흐흐흐흐흐.", false, false)
        else
            me:dialog(npc, "자네! 대체 어디서 그런 소리를 들은겐가! 이것은 내가 인심을 팍팍 써줘서 15만전이지 딴데 가서는 어디서 팔지도 못해! 음음.. 내가 특별히 15만전에 사주는데도 어디서 헛소리를 들어 왔는진 모르겠군..", false, false)
        end
        return
    end
    if sel == 1 then
