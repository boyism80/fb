...
function NPC_336(me, npc)
    ...
    if day >= 1 and day <= 24 then
        ...
        btn = me:dialog(npc, "오늘은 [" .. item_name .. "] " .. name_with(item_name, '을', '를') .. " 구입 가능합니다.", false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if me:has_items(item_name, 1) then
            me:dialog(npc, "이미 [" .. item_name .. "] " .. name_with(item_name, '을', '를') .. " 가지고 계신것 같군요. 저는 욕심쟁이를 좋아하지 않아요.", false, false)
            return
        end
    ...
    if day == 25 or day == 26 then
        me:dialog(npc, "오늘은 원하는 색상의 도톨 하나를 드리고 있어요. 단...! 가격은 비싸다는 점! 참고해 주세요. (해당 기능은 준비중입니다.)", false, false)
        return
    end
    ...
    me:dialog(npc, "와, 정말 모든 도톨을 다 모아 오셨군요! 그럼 칠지도로 교환 해 드릴게요.", false, true)
    me:dialog(npc, "여기 있습니다. 잘 쓰시길 바라며, 수행이 필요 없을지라도 꾸준한 수행은 자신의 진의를 깨달을 수 있는 길입니다. 나무아미타불..", false, false)
end
