...
function NPC_450(me, npc)
    ...
    if now_ts and now_ts > 0 and next_ts > now_ts then
        ...
        me:dialog(npc, string.format("일전에는 정말 감사했습니다. 곧 잔가지가 다시 필요할 것 같으니 %d시간 %d분 후에 다시 와주세요!", hour, min), false, false)
        return
    end

    local btn = me:dialog(npc, string.format("앗...! 안녕하십니까 %s님. 저는 북방나무꾼이라고 합니다.", me:name()), false, true)
    ...
    if sel == 3 then
        ...
        me:dialog(npc, "어디까지나 자잘한 가지들을 모으는 것이기 때문에, 곳곳에 있는 나무들에게 다가가 잔가지를 주워 오시면 됩니다. ", false, true)
        me:dialog(npc, "나무들은 맨손으로 공격하셔도 좋고 마법으로 공격하셔도 좋습니다.\n\n그렇게 나무 주위에서 떨어지는 잔가지들을 구해다 주시면 됩니다...", false, false)
        return
    end
end
