...
local function buy_clan_sword(me, npc)
    ...
    if castle == nil or castle:owner_clan() ~= clan:id() then
        me:dialog(npc, '이 성을 가진 문파장에게만 자격이 있다네.')
        return
    end

    local weapon_name = castle_lib.WEAPON_NAME_BY_DIVINE_BEAST[divine_beast]
    if #me:items(weapon_name) > 0 then
        me:dialog(npc, '자네는 이미 이 성을 대표하는 검을 가지고 있는 것 같군..')
        return
    end
    ...
    me:dialog(npc, '이 성을 가진 주인이라면 마땅히 그 힘을 가질 자격이 있지.')
    me:dialog(npc, '그러나 그 힘을 얻기 위해서는 그에 마땅한 댓가로 따르는 법...')

    local OPT_YES = '네. 지금 당장 주세요!'
    local OPT_NO = '아니요. 돈이 없네요...'
    local selected = me:pursuit(npc, string.format('그래, 금전 %d전을 댓가로 문파검을 구입하겠는가?', price), { OPT_YES, OPT_NO })
    if selected ~= OPT_YES then
        return
    end

    if me:money() < price then
        me:dialog(npc, '감히 어디서 거짓말인가!!')
        return
    end

    if me:mkitem(weapon_name, 1) == nil then
        me:dialog(npc, '소지품이 가득 찼습니다.')
        return
    end

    me:money(me:money() - price)
    me:dialog(npc, '이 검은 극도로 강한 기운을 잡아놓아 무척이나 불안하다네.')
    me:dialog(npc, '죽으면 그 힘이 모두 사라지게 되니 조심해서 쓰시게나.')
end
...
return {
    on_click = function(me, npc)
        ...
        if button == DIALOG_RESULT.QUIT or selected ~= OPT_BUY then
            return
        end
