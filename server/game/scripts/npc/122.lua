...
return {
    on_click = function(me, npc)
        ...
        if dq and not dq:completed() and dq:step() == 5 then
            ...
            if sel == 1 then
                ::NPC_122_0000::
                local btn = me:dialog(npc, "그건 모조품이라구! 아니, 그럼 내가 진짜를 줬을 줄 알았어? 허참! 어이가 없구만!", { prev = false, next = true })
                if btn == DIALOG_RESULT.QUIT then
                    return
                end
                btn = me:dialog(npc, "그리고 교역허가? 허가를 내줄 공무원들이 맨날 숲에서 낮잠이나 자고 있는데, 무슨 수로 허가를 맡느냔 말이지!", { prev = true, next = true })
                if btn == DIALOG_RESULT.QUIT then
                    return
                end
                ...
                dq:step(6)
                me:dialog(npc, "에이! 진진처럼 게으르고 굼뜨고 못미더운 사람들 같으니라구!", { prev = false, next = false })
            elseif sel == 2 then
                me:dialog(npc, "준비중입니다.", { prev = false, next = false })
            else
                me:dialog(npc, "그러시군요.", { prev = false, next = false })
            end
            return
        end
        ...
        if q == nil then
            local btn = me:dialog(npc, '안녕하신가, 나는 무기 수집가라네. 자네 대륙 사람 맞지?', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
        ...
        local btn = me:dialog(npc, '과연! 묵직하고 단단한 것이 무기로도 손색이 없겠어!', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
