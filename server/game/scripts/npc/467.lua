...
local function do_sub4_return_baby(me, npc)
    if not me:has_items("아기원숭이", 1) then
        me:dialog(npc, "퀘스트 오류입니다.\n아기원숭이 아이템이 없습니다.", { prev = false, next = false })
        return true
    end
    ::NPC_467_0040::
    local b = me:dialog(npc, "잠깐만! 자네가 안고 계시는 그 아기 원숭이는 누구의 자식이시지? 뭐? 어머니를 찾고 계시다고?", { prev = false, next = true })
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    ::NPC_467_0041::
    b = me:dialog(npc, "밥도 안드시고 실의에 빠져계시는 원숭이 부부가 있으셨다. 이런 기쁘실데가 있나! 수고 하셨다!", { prev = true, next = true })
...
local function do_sub5_start(me, npc)
    ...
    ::NPC_467_0030::
    local b = me:dialog(npc, "요즘 날쌔시고 유능하신 부하들을 시켜서서 다른 왕들과 편지를 주고받으시고 계신데, 그만 이번에 중요한 편지", { prev = true, next = true })
    if b == DIALOG_RESULT.PREV then
        goto NPC_467_0020
    end
    if b == DIALOG_RESULT.QUIT then
    ...
    ::NPC_467_0031::
    b = me:dialog(npc, "하나가 흉폭해진 원숭이들 손에 떨어지고 마셨다. 부하들의 말에 따르면 요 앞에서 잃어버리셨다고하니, 우리", { prev = true, next = true })
    if b == DIALOG_RESULT.PREV then
        goto NPC_467_0030
    end
    if b == DIALOG_RESULT.QUIT then
    ...
    ::NPC_467_0032::
    b = me:dialog(npc, "부족이 사시는 이 건물안에 남아계신 것은 확실하시다. 자네가 어떻게 좀 찾아주셨으면 좋으시겠다.", { prev = false, next = true })
    if b == DIALOG_RESULT.PREV then
        goto NPC_467_0031
    end
    if b == DIALOG_RESULT.QUIT then
...
local function do_sub5_turnin(me, npc)
    if not me:has_items("원숭이편지", 1) then
        me:dialog(npc, "부족이 사시는 이 건물안에 남아계신 것은 확실하시다.", { prev = false, next = false })
        return true
    end
    ::NPC_467_0050::
    local b = me:dialog(npc, "이런 편지를 찾아오시다니 엄청난 실력이시군. 도대체 어디서 찾으신건가 믿을 수가 없으시다.", { prev = false, next = true })
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    ::NPC_467_0051::
    b = me:dialog(npc, "분명 이걸 찾아오시느라 많은 고생을 하셨겠지? 자, 이걸 드시면 좋으시다. 몸이 허하실때 드시는", { prev = false, next = true })
    if b == DIALOG_RESULT.PREV then
        goto NPC_467_0050
    end
    if b == DIALOG_RESULT.QUIT then
