...
return {
    on_click = function(me, npc)
        ...
        if q == nil then
            ...
            ::NPC_138_0011::
            btn = me:dialog(npc, '이들 산신은 인간이었을때 가지고 있던 소지품을 어떤곳에 묻어서 버린다고 하는데, 신선수행을 하던 사람들이었으니 귀중품들이 많이 있을겁니다.', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            ::NPC_138_0012::
            btn = me:dialog(npc, '도삭산 801층 이후에는 책장들이 있는 곳이 있는데, 그중 어딘가 한군데에서 제가 보물지도를 본적이 있습니다. 아마 산신들의 귀중품을 숨겨놓은 곳을 표시해둔 것 같은데, 그때 도망치느라 가져오질 못했습니다.', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            local sel, list_btn = me:list(npc, '한번 구하러 가보시지요?', { '네, 찾아가봐야 겠군요.', '갈길이 바빠서..' }, { prev = false })
        ...
        if step == 2 then
            if not me:has_items(ITEM_FABRIC, 1) then
                me:dialog(npc, '보물지도를 사용하면 숨겨진 곳이 적혀 있습니다. 그곳으로 가보세요.', { prev = false, next = false })
                return
            end
            local btn = me:dialog(npc, '오오! 이것은 산신의비단이 아닙니까! 이걸 저에게 맡기시면 제가 멋진 옷을 만들어드리겠습니다!', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            local sel, list_btn = me:list(npc, '저에게 맡기시는 것이 어떠실지?', { '네 그럼 부탁합니다.', '안됩니다 어떻게 구한건데.' }, { prev = false })
            if list_btn == DIALOG_RESULT.QUIT then
                return
            end
            if sel ~= 1 then
            ...
            ::NPC_138_0020::
            btn = me:dialog(npc, '크.. 이런 옷감을 내 손으로 만지게 되다니.. 영광입니다 영광..', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_138_0021::
            btn = me:dialog(npc, '그런데 이 옷감.. 너무 튼튼해서 보통 바늘로는 꿰멜수가 없겠는데요.. 아무래도 산신들이 이 옷감을 다룰때 쓰는 바늘이 있을 것 같은데..\n\n산신들이 사용하는 바늘이 있어야 옷을 만들 수 있겠습니다..', { prev = false, next = true })
            if btn == DIALOG_RESULT.PREV then
                goto NPC_138_0020
            end
            if btn == DIALOG_RESULT.QUIT then
        ...
        if step == 3 then
            ...
            ::NPC_138_0030::
            local btn = me:dialog(npc, '이것은... 바늘이 아니라 새털처럼 보이는데.. 어디 한번..', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_138_0031::
            btn = me:dialog(npc, '아니! 이것은 마법이 걸린 깃털인가 봅니다. 어째서 이런 튼튼한 옷감이 이런 부드러운 것에 구멍이 나는 것인지...', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            ::NPC_138_0032::
            btn = me:dialog(npc, '자.. 그럼 어디한번 옷을 만들어볼까요.. 아. 그런데 어떤 옷을 만들어드리지요?', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            ::NPC_138_0040::
            btn = me:dialog(npc, name_with(item_name, '는', '은') .. ' ' .. gender .. ' ' .. class_name .. '들을 위한 옷입니다.', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_138_0041::
            btn = me:dialog(npc, '자.. 그럼.. ...... ...... ..........', { prev = false, next = true })
            if btn == DIALOG_RESULT.PREV then
                goto NPC_138_0040
            end
            if btn == DIALOG_RESULT.QUIT then
