...
return {
    on_click = function(me, npc)
        ...
        if dq and not dq:completed() then
            ...
            if ds == 7 then
                ::NPC_118_0001::
                local btn = me:dialog(npc, "으잉? 그...그건 수..수사관 명찰 아닌가요? 도..도대체 무슨 일이죠?", { prev = false, next = true })
                ...
                if sel == 1 then
                    dq:step(8)
                    me:dialog(npc, "아..아니, 난 별로 할 말..두 없구! 아! 바빠! 바쁘다니까!", { prev = false, next = false })
                else
                    me:dialog(npc, "빨리 지..지나가슈!!", { prev = false, next = false })
                end
                return
            end
            if ds == 9 then
                ...
                if sel == 1 or sel == 3 or sel == 4 then
                    me:dialog(npc, "어..얼른 가슈!", { prev = false, next = false })
                    return
                end
                ...
                if sel == 1 then
                    me:dialog(npc, "나..난 정말 죄 없어!", { prev = false, next = false })
                    return
                end
                ::NPC_118_0009::
                local btn = me:dialog(npc, "어..어쩔 수 없었어! 우리 어머니를 좀 더 잘 모시고 싶었을뿐이야!", { prev = false, next = true })
                if btn == DIALOG_RESULT.QUIT then
                    return
                end
                ::NPC_118_0010::
                btn = me:dialog(npc, "그냥.. 그냥 시키는대로 궤짝을 날라다 배에 실어준 것 밖에 없다구..", { prev = true, next = true })
                if btn == DIALOG_RESULT.QUIT then
                    return
                end
                ...
                btn = me:dialog(npc, "(진진은 얼굴을 감싸쥐고 울기 시작했다.)", { prev = false, next = true })
                if btn == DIALOG_RESULT.QUIT then
                    return
                end
                dq:step(10)
                me:dialog(npc, "(당신은 이번 사건에서 진진의 증언만을 기억하고, 진진의 죄는 잊기로 마음 먹었다.)", { prev = false, next = false })
                return
            end
