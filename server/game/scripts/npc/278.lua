...
function NPC_278(me, npc)
    ...
    button = me:dialog(npc, "기본적인 것들은 앞선 도우미들에게 모두 배우셨죠?", { prev = false, next = true })
    if button == DIALOG_RESULT.QUIT then
        return
    end

    button = me:dialog(npc, "저는 궁금한 것이 있다면 답변해 드리겠습니다.", { prev = false, next = true })
    if button == DIALOG_RESULT.QUIT then
        return
    end

    button = me:dialog(npc, "열심히 공부한 학생에게는 상이 있을수도 있습니다.", { prev = false, next = true })
    if button == DIALOG_RESULT.QUIT then
        return
    end
    ...
    ::NPC_278_LIST::
    local sel, btn = me:list(npc, "그럼. 아래 중에서 궁금하신것을 선택해 주세요. 없으면 맨 아래의 [그만]을 눌러주세요.", faq, { prev = false })
    ...
    if sel == 8 then
        button = me:dialog(npc, "바람의나라에서 신수란 각각 사방을 상징하는 '좌-청룡', '우-백호', '북-주작', '남-현무'로 구성됩니다.", { prev = false, next = true })
        if button == DIALOG_RESULT.QUIT then
            return
        end
        button = me:dialog(npc, "신수는 게임 캐릭터를 생성할 때 선택하실 수 있고, 신수에 따라 마법 공격 이미지가 달라지게 됩니다.", { prev = false, next = true })
        if button == DIALOG_RESULT.QUIT then
            return
        end
