...
local function run_pardon_placeholder(me, npc)
    me:dialog(npc, "사면 기능은 준비중입니다.", { prev = false, next = false })
end
...
local function run_nation_change(me, npc)
    ...
    ::NPC_274_0001::
    local btn = me:dialog(npc, "내 비록 지금은 귀양을 온 처지이지만, 그 정도는 할 수 있다네... 어디보자...", { prev = false, next = true })
    ...
    if me:money() < cost then
        me:dialog(npc, "저런, 재물이 모자라는군, 다음에 다시 오게나... 그럼...", { prev = false, next = false })
        return true
    end
    me:money(me:money() - cost)
    me:nation(target_nation)
    btn = me:dialog(npc, "그대의 노고를 높이 사 상제께서 그대의 소원을 들어주셨네.", { prev = false, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return true
    end
    me:dialog(npc, "이제 " .. target_name .. "인 답게 살아가게나.", { prev = false, next = false })
    return true
end

local function run_weapon_three_dialogs(me, npc, line1, line2, line3)
    local btn = me:dialog(npc, line1, { prev = false, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return false
    end
    btn = me:dialog(npc, line2, { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return false
    end
    ...
    me:dialog(npc, line3, { prev = false, next = false })
    return true
end

local function run_weapon_info_loop(me, npc)
    ::NPC_274_0010::
    local btn = me:dialog(npc, "나는 이 세상에 존재하는 모든 무기에 대해 알고 있네. 또한 몇몇 무기는 만들 수도 있다네... 재료만 있다면...", { prev = false, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    ...
    while true do
        ...
        if weapon_sel == 8 then
            me:dialog(npc, "잘 가게나...", { prev = false, next = false })
            return
        end
        ...
        elseif weapon_sel == 5 then
            me:dialog(npc, "백화검이라... 백화검을 만드는 데에는 세가지 재료가 필요하다네... 하지만 지금은 때가 아니야... 미안하네...", { prev = false, next = false })
        elseif weapon_sel == 6 then
            me:dialog(npc, "빙룡검은 백화검과 정반대의 속성을 지닌 검일세... 하지만 역시 지금은 때가 아닐세... 미안하네...", { prev = false, next = false })
        else
            local r = run_weapon_three_dialogs(me, npc,
