...
local function do_hair_style(me, npc)
    ...
    while true do
        ...
        local sel = me:list(appearance, msg, {'이 모습을 선택하겠습니다.', '다른 머리모양을 보여주세요.'}, { prev = false })
        if sel == nil then
            return
        end
        ...
        if sel == 1 then
            ...
            if money < price then
                me:dialog(npc, '금액이 ' .. (price - money) .. '전 부족합니다.', { prev = false, next = false })
                return
            end
            me:look(face)
...
local function do_hair_dye(me, npc)
    ...
    local money = me:money()
    if money < cost then
        me:dialog(npc, '금액이 ' .. (cost - money) .. '전 부족합니다.', { prev = false, next = false })
        return
    end
