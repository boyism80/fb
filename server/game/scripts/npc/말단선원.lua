...
function NPC_145(me, npc)
    ...
    if quest == nil or quest:step() < 1 then
        ::COS001::
        local sel, btn = me:list(npc, '아이고~ 이거 큰일이네! 어떡하지? 엉엉 울고싶다 정말.', { '무슨 일이세요?', '(그냥 지나친다)' }, true)
        ...
        ::COS003::
        sel, btn = me:list(npc, '아 글쎄.. 이번엔 제가 애써 모아놓은 기러기알들을 그놈들이 다 훔쳐갔지 뭡니까요! 흑흑. 그걸 어떻게 되찾는다.', { '흠. 그거라면.. 제가 그놈들을 해치우고 되찾아 드리지요.', '어쩔 수 없죠 뭐..' }, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
