...
function NPC_357(me, npc)
    ...
    button = me:dialog(npc, "몽연이가 청소를 한다구요?", false, true)
    if button == DIALOG_RESULT.QUIT then
        return
    end
    button = me:dialog(npc, "작년 추석에 송편 만들다가 냄비를 떨어뜨려서 냄비가 찌그러졌는데.. 잘됐네요! 나중에 선릉이한테 냄비 새로 만들어 달래야지~", false, true)
    if button == DIALOG_RESULT.QUIT then
        return
    end
    button = me:dialog(npc, "감사합니다!!", false, true)
    if button == DIALOG_RESULT.QUIT then
        return
    end
    me:mkitem("찌그러진냄비", 1)
