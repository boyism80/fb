...
local function run_turn_in(me, npc)
    ...
    if me:has_items(ITEM_OLD_LETTER, 1) then
        local sel, btn = me:list(npc, '아직 멀었나?', { '아뇨, 다 됐습니다.', '예, 아직 멀었어요.' }, false)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if sel == nil then
    ...
    me:dialog(npc, '아직 멀었나 보군..', false, false)
end
...
local function run_cursed_forgiveness(me, npc)
    ...
    local sel, btn = me:list(npc, '에이! 네놈만 보면 화가 난다. 설마 했지만 그래도 믿었는데, 보란 듯이 약속을 어기다니! 저리 가! 꼴도 보기 싫다!', { '......', '죄송합니다. 호기심에 그만...' }, false)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if sel == nil then
