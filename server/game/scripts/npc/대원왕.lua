...
local function do_sub7_cure(me, npc)
    ...
    local b = me:dialog(npc, "연락은 진작부터 듣고 계셨다. 자, 어서 아이를 이리 주셔야 좋으시다. 나의 힘으로 열병따윈 순식간에 치료해주신다!", false, true)
    if b ~= 1 then
        return false
    end
    b = me:dialog(npc, "나 대원왕의 힘을 보시는게 좋으시다! 하아아앙아아앗!", true, true)
    ...
    b = me:dialog(npc, "헉헉, 자, 씻으신듯이 열이 가라앉으셨다. 어서 이 기쁜 소식을 부모에게 전해주셔야 좋으시다.", true, true)
    if b == DIALOG_RESULT.PREV then
        return false
    end
    if b == DIALOG_RESULT.QUIT or not b then
...
local function do_sub1_start(me, npc)
    ...
    b = me:dialog(npc, "찌꺼기 과일을 가져오신 인간들에게 있으시다. 반성하셔야 한다. 이 땅에 오신 이상, 우리도 적응하셔야 하지만,", true, true)
    if b == DIALOG_RESULT.PREV then
        return false
    end
    if b == DIALOG_RESULT.QUIT or not b then
    ...
    b = me:dialog(npc, "무작정 이 땅의 과일만 주시는건 해서는 안될 일이시다. 아이들을 잃으셔서 가슴이 아프시다. 아프시다.", true, true)
    if b == DIALOG_RESULT.PREV then
        return false
    end
    if b == DIALOG_RESULT.QUIT or not b then
    ...
    local sel, btn = me:list(npc, "요즘따라 인간들을 자주 만나시게 되시는도다.", { "이름을 가르쳐주세요.", "이곳의 상황은 좀 어떤가요?", "제가 도와드릴 일은 없을까요?" }, true)
    ...
    b = me:dialog(npc, "고구려나 부여에서는 얼음이라는걸 구하실 수가 있으시다.", true, true)
    if b == DIALOG_RESULT.PREV then
        return false
    end
    if b == DIALOG_RESULT.QUIT or not b then
    ...
    b = me:dialog(npc, "투명하시고 차가우시면서도 딱딱하신 물체라고 하시는데... 뜨거운 섬에서 자라나신 나로서는 상상조차", true, true)
    if b == DIALOG_RESULT.PREV then
        return false
    end
    if b == DIALOG_RESULT.QUIT or not b then
    ...
    b = me:dialog(npc, "할 수가 없으시다. 나에게 그 얼음이라는걸 보여주시면 좋으시다. 깨끗하시고 투명하신 분으로 말이시다.", true, true)
    if b == DIALOG_RESULT.PREV then
        return false
    end
    if b == DIALOG_RESULT.QUIT or not b then
...
local function do_sub1_turnin(me, npc)
    ...
    local b = me:dialog(npc, "오오. 이런 신기하신 것이 다 계실 줄이야! 이 곳은 정말 놀라움으로 가득찬 곳이시다. 하하하하하.", false, true)
    if b ~= 1 then
        return false
    end
    b = me:dialog(npc, "과연과연이시다. 생김새도 아름다우시고, 입에 넣으시면 시원시원하시고, 손에 쥐고계시면 차가우시다.", true, true)
    if b == DIALOG_RESULT.PREV then
        return false
    end
    if b == DIALOG_RESULT.QUIT or not b then
    ...
    b = me:dialog(npc, "자, 이건 내가 드리시는 답례이시다. 이 땅에서는 보기 힘드신 과일로 만드신 음식이시다. 맛있게 드시면 좋으시다.", true, true)
    if b == DIALOG_RESULT.PREV then
        return false
    end
    if b == DIALOG_RESULT.QUIT or not b then
...
function NPC_463(me, npc)
    ...
    if main_q then
        ...
        if s == 9 then
            me:dialog(npc, "참원왕에게 내 의견을 전달해주시면 되시다.", false, false)
            return
        end
        if s == 12 then
        ...
        if s == 14 then
            me:dialog(npc, "남으신 한 병은 참원왕께 가져다주시면 고마우시다.", false, false)
            return
        end
        if s == 27 then
