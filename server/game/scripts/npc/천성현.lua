...
function NPC_90(me, npc)
    ...
    if me:promotion() < 1 then
        me:dialog(npc, '2차 승급을 한 뒤에 다시 오세요.', false, true)
        return
    end
    ...
    if quest == nil then
        ...
        if me:dialog(npc, '길림성이란 곳이 있습니다.', false, true) == DIALOG_RESULT.QUIT then
            return
        end
        if me:dialog(npc, '이곳은 청룡 백호, 주작 현무의 신전이 각 방위마다 붙어있으며 조화로운 곳으로 알려져있습니다.', false, true) == DIALOG_RESULT.QUIT then
            return
        end
        if me:dialog(npc, '그런데 어쩐일인지 이들 신수들의 힘이 예전보다 약해지고 있는데 이유를 알아내지 못하고 있습니다.', false, true) == DIALOG_RESULT.QUIT then
            return
        end
        if me:dialog(npc, '그리고 길림성에 살고 있던 동물들도 이상하게 변했다고 하더군요.', false, true) == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if me:dialog(npc, '그럼 제가 당신에게 부탁했다는 증거로 증표를 드리겠습니다.', false, true) == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if me:dialog(npc, '그럼 길림성으로 가보십시오. 거기서 당신의 신수의 제단을 찾아가 계시를 받아야 할 것입니다.', false, true) == DIALOG_RESULT.QUIT then
            return
        end
        if me:dialog(npc, '각 신수의 제단은 동쪽에 청룡의 제단, 서쪽에 백호의 제단, 남쪽에 주작의 제단, 북쪽에 현무의 제단이 있습니다.', false, true) == DIALOG_RESULT.QUIT then
            return
        end
        if me:dialog(npc, '그럼 성공하시길 잘 다녀오시길 바랍니다..', false, true) == DIALOG_RESULT.QUIT then
            return
        end
        return
    end
    ...
    if quest:step() == 1 then
        ...
        for _, msg in ipairs(dialogs) do
            if me:dialog(npc, msg, false, true) == DIALOG_RESULT.QUIT then
                return
            end
        end
    end
    ...
    if quest:step() == 2 then
        ...
        if me:dialog(npc, '아.. 성공하셨군요!! 당신이 암흑의 존재를 처치한 것을 옥황상제님께서도 기뻐하실 것입니다.', false, true) == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if me:dialog(npc, '천상제일계의 선녀들에게로 가 보십시오.', false, true) == DIALOG_RESULT.QUIT then
            return
        end
    end
end
