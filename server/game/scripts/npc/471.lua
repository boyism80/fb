...
local function do_sub9_start(me, npc)
    ...
    ::NPC_471_0001::
    local b = me:dialog(npc, "내가 이번에 새로 술을 담그셨다. 친구에게 술을 주고 싶으시다.", { prev = true, next = true })
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    ::NPC_471_0002::
    b = me:dialog(npc, "하지만, 이런 비상시에 왕이 자리를 비우실 수는 없으셔서 곤란하셨던 참이시다.", { prev = true, next = true })
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    ...
    if q9 == nil then
        q9 = me:start_quest(quest.QUEST_SKULL_NECKLACE_9)
        if q9 == nil then
            return true
        end
    end
...
local function do_sub9_complete(me, npc)
    ::NPC_471_0004::
    local b = me:dialog(npc, "아, 정말 수고하셨다. 보답으로 이걸 드리시겠다. 내가 털을 다듬으실때, 쓰시는 물건이시다.", { prev = false, next = true })
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    ::NPC_471_0005::
