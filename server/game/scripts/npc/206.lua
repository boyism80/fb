...
local function run_schedule_dialog_japan(me, npc)
    ::NPC_206_0010::
    local btn = me:dialog(npc, '고구려에서 일본으로 건너가는 배는 하루 총 12번입니다.', { prev = false, next = true })
    ...
    ::NPC_206_0012::
    btn = me:dialog(npc, '출항하기전 미리 배에 오르고 있어야만 일본으로 갈 수 있음을 명심하시기 바랍니다.', { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if btn == DIALOG_RESULT.PREV then
...
return {
    on_click = function(me, npc)
        ...
        if sel == 1 then
            run_schedule_dialog_japan(me, npc)
            return
        end
        if sel ~= 2 then
            return
        ...
        if not me:has_items('고구려-일본승선권', 1) then
            me:dialog(npc, '승선권이 없으면 선실에 탑승하실 수 없습니다.', { prev = false, next = false })
            return
        end
        if not me:rmitem('고구려-일본승선권', 1, ITEM_DELETE_TYPE.GIVE) then
            return
        end

        -- Ticket already consumed: do not allow QUIT to skip boarding.
        me:dialog(npc, '그럼, 선실에 승선시켜드리도록 하겠습니다.', { prev = false, next = true })
        me:dialog(npc, '배가 출항한뒤 일정 시간 후에 일본에 도착하게 됩니다. 즐거운 여행 되시길..', { prev = false, next = false })

        local map = name2map('고구려-일본배선실')
        if map ~= nil then
