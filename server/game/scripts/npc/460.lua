    ...
    ON_CLICK = function(me, npc)
        if property("check") == 25 and me:has_items("훈제연어", 5) then
            ...
            if sel == 1 then
                ...
                me:dialog(npc, "자, 당근케이크를 받았으니... 다시 임무를 수행하러 가봐도 된다웅!!", { prev = false, next = false })
            end
            return
        end

        local btn = me:dialog(npc, "난 깊은계곡을 지키고 있는 곰이다웅...", { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        btn = me:dialog(npc, "연어가 먹고싶은데... 맛있는연어를 먹으면 혼날것 같고... 혹시 가지고 있는 훈제연어라도 있냐웅?", { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if sel == 5 then
            me:dialog(npc, "세상의 다양한 물건을 모으고 있으니... 다음에 새로 모은게 있으면 알려주겠다웅.", { prev = false, next = false })
            return
        end

        me:dialog(npc, "해당 교환은 준비중입니다. 홈페이지에 문의해 주세요.", { prev = false, next = false })
    end
}
