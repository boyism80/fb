...
local function do_sub8_start(me, npc)
    ...
    ::NPC_470_0010::
    local b = me:dialog(npc, "고마우시다. 내가 아직 병에 걸리시지 않은 어린 원숭이들을 구하시러 돌아다니시다가 그만 병에 걸리신", true, true)
    ...
    ::NPC_470_0011::
    b = me:dialog(npc, "원숭이들에게 왕의 증표를 빼앗기고 마셨다. 너무 흉폭하셔 내 힘으로는 되찾으실 도리가 없으시다.", true, true)
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    if b == DIALOG_RESULT.PREV then
    ...
    b = me:dialog(npc, "그걸 자네가 도로 되찾아주셨으면 좋으시겠군.", false, true)
    if b == DIALOG_RESULT.QUIT then
        return false
    end
...
local function do_sub8_turnin(me, npc)
    ...
    ::NPC_470_0020::
    local b = me:dialog(npc, "오, 이런 고마우실데가 계실까! 음, 내가 자네를 믿으시길 잘하셨네! 답례를 하나 하시겠네!", false, true)
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    b = me:dialog(npc, "이건 내가 요 앞에 보초를 서고있으시는 병사들의 방패를 흉내내시어 만들어보신 물건이시네.", false, true)
    if b == DIALOG_RESULT.QUIT then
        return false
    end
...
function NPC_470(me, npc)
    ...
    if main_q then
        ...
        if s == 16 then
            ::NPC_470_0030::
            local b = me:dialog(npc, "뭐? 참원왕께서 의견을 물어보고 계신다는 것이신가?", false, true)
            if b == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_470_0031::
            b = me:dialog(npc, "흠, 마침 좋은 생각을 하나 떠올리셨다. 지금 흉악해진 원숭이들로부터 우리 원숭이들도 몸을 지켜야만하시지.", true, true)
            if b == DIALOG_RESULT.QUIT then
                return
            end
            if b == DIALOG_RESULT.PREV then
            ...
            b = me:dialog(npc, "해서 나는 우리 원숭이들이 스스로를 지키실 수 있도록 무장을 하실 필요가 있다고 생각하신다. 인간들처럼 말이시다.", false, true)
            if b == DIALOG_RESULT.QUIT then
                return
            end
            main_q:step(17)
        ...
        if s == 17 then
            me:dialog(npc, "참원왕에게 내 의견을 전해주셨으면 하시다.", false, false)
            return
        end
        if s == 18 then
            if not me:has_items("철도", 3) or not me:has_items("사각방패", 3) then
                me:dialog(npc, "철도 3개와 사각방패 3개정도면 자경단을 꾸릴 수 있을 것 같으시다.", false, false)
                return
            end
            ::NPC_470_0040::
            local b = me:dialog(npc, "정말 수고하셨다. 멋진 무기들이시다. 이것을 기반으로 우리 원숭이들도 스스로를 지키실 수 있는 힘을 얻으셨다.", false, true)
            if b == DIALOG_RESULT.QUIT then
                return
            end
            b = me:dialog(npc, "걱정하지 않으셔도 좋으시다. 인간들과 싸우지 않으실 계획이시다. 자, 그러면 이 기쁜 소식을 참원왕께도 빨리 전해주셔야한다.", false, true)
            if b == DIALOG_RESULT.QUIT then
                return
            end
        ...
        if s == 19 then
            me:dialog(npc, "이 기쁜 소식을 참원왕에게도 전해주셔야 한다.", false, false)
            return
        end
        if s == 31 then
