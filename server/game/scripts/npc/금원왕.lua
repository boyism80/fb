...
local function do_sub8_start(me, npc)
    ...
    b = me:dialog(npc, "그걸 자네가 도로 되찾아주셨으면 좋으시겠군.", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
...
local function do_sub8_turnin(me, npc)
    ...
    b = me:dialog(npc, "이건 내가 요 앞에 보초를 서고있으시는 병사들의 방패를 흉내내시어 만들어보신 물건이시네.", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
...
function NPC_470(me, npc)
    ...
    if main_q then
        ...
        if s == 16 then
            ...
            b = me:dialog(npc, "해서 나는 우리 원숭이들이 스스로를 지키실 수 있도록 무장을 하실 필요가 있다고 생각하신다. 인간들처럼 말이시다.", true, true)
            if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
                return
            end
            main_q:step(17)
        ...
        if s == 18 then
            ...
            b = me:dialog(npc, "걱정하지 않으셔도 좋으시다. 인간들과 싸우지 않으실 계획이시다. 자, 그러면 이 기쁜 소식을 참원왕께도 빨리 전해주셔야한다.", true, true)
            if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
                return
            end
