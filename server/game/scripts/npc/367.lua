...
local function run_quest_accept(me, npc)
    ::NPC_367_0010::
    local btn = me:dialog(npc, "정말인가? 자네가 유물 발굴을 도와주면, 우리도 뭔가 보답을 하겠네.", { prev = false, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    ::NPC_367_0011::
    btn = me:dialog(npc, "무엇을 주면 좋을까... 아 그렇지! 고대유물의 조각에는 귀한 금속의 결정이 조금씩 섞여 있는 경우가 있는데, 그것을 자네에게 주겠네.", { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    ...
    ::NPC_367_0012::
    btn = me:dialog(npc, "요새는 남양과 남경의 굴에 있는 괴물들이 가지고 다니고 있지. 구슬조각의 형태를 하고 있다네.", { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if btn == DIALOG_RESULT.PREV then
    ...
    ::NPC_367_0013::
    btn = me:dialog(npc, "갈색과 자색의 구슬조각은 양의 속성을 가지고 있고, 청색과 녹색의 구슬조각은 음의 속성을 가지고 있지.", { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if btn == DIALOG_RESULT.PREV then
    ...
    ::NPC_367_0014::
    btn = me:dialog(npc, "이 구슬조각에는 귀환 금속의 결정이 조금씩 섞여 있는 경우가 있는데, 그것을 자네에게 주겠네. 우리 조사단에는 필요가 없는 물건이거든.", { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if btn == DIALOG_RESULT.PREV then
    ...
    ::NPC_367_0015::
    btn = me:dialog(npc, "그럼 주의하면서 구슬조각을 구해다 주겠는가? 한꺼번에 많이 가져올수록 좋다네. 그럼 잘 부탁하겠네.", { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if btn == DIALOG_RESULT.PREV then
...
local function run_bead_exchange(me, npc)
    ...
    if sel == 2 then
        me:dialog(npc, "그렇군. 힘내서 좀 도와주게나..", { prev = false, next = false })
        return
    end
    if sel ~= 1 then
        return
    ...
    if not me:has_items(bead_item, count) then
        local btn = me:dialog(npc, bead_item .. " " .. name_with(bead_item, '이', '가') .. " 부족하지 않은가?", { prev = false, next = false })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        return
    end
    local btn = me:dialog(npc, "오, 정말 가져왔군!\\n\\n이것을 결정으로 합쳐보겠네. 잠시만 기다리시게나.", { prev = false, next = false })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    ...
    me:dialog(npc, "자, 여기있네.", { prev = false, next = false })
end
...
local function run_intro_and_list(me, npc)
    ::NPC_367_0000::
    local btn = me:dialog(npc, "남경과 남양은 고대 유물의 보고야. 누구라도 세심하게 조사하면 어디에서나 유물이 나올 정도라네. 그래서 우리 조사단은 여기를 근거지로 고대유물을 발굴하고 있지.", { prev = false, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    ::NPC_367_0001::
    btn = me:dialog(npc, "하지만, 요새는 괴물들이 자주 출몰하여 위험해 지원자가 없지. 그래서 항상 사람이 모자란다네. 오늘도 두명이 괴물을 보자마자 도망쳤는데, 참 걱정일세..", { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    if btn == DIALOG_RESULT.PREV then
