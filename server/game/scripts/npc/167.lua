...
return {
    on_click = function(me, npc)
        ...
        ::NPC_167_0001::
        local btn = me:dialog(npc, '안녕, 보시다시피 나는 사냥꾼이야. 요즘 잡스러운 괴물들이 다시 들끓더니 외부인까지 들어오더만\n\n뭐 나쁜 일만은 아니야. 괴물들을 상대하려면 사람들이 많이 필요하거든. 이를테면 자네가 도와준다거나 할 수 있으니까.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end

        ::NPC_167_0002::
        btn = me:dialog(npc, '아무튼 자네의 두 눈에 정의감이 흘러넘치는걸 보니 괴물들과 싸울 작정인가 보군. 그래, 그럴거야.\n\n그렇다면, 나도 가만히 있을수는 없지. 우리 가문에 대대로 전해져 내려오는 가문의 비밀 무기들을 만들어주지', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ...
        ::NPC_167_0003::
        btn = me:dialog(npc, '각각 거미독창과 저주의단검이라 불리는 무기들일세. 갖고 싶다면 재료만 좀 구해오면 되고... 또 돈도 조금만 주면 끝이지. 후후후후', { prev = true, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        if btn == DIALOG_RESULT.PREV then
