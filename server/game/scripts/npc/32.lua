...
return {
    on_click = function(me, npc)
        ...
        ::NPC_32_0001::
        local b = me:dialog(npc, "뭐? 문자를 배울 수 있는 책? 창힐독본말인가? 음, 그건 왕실 학문관에서만 볼 수 있는 책일세", { prev = false, next = true })
        ...
        ::NPC_32_0003::
        b = me:dialog(npc, "내가 손주에게 보여주려고 빌려둔 책이 두어권 있는데, 이걸 자네에게 주겠네.", { prev = true, next = true })
        if b == DIALOG_RESULT.QUIT then
            return
        end
        ...
        ::NPC_32_0004::
        b = me:dialog(npc, "다만, 책을 분실하면 왕실 학문관에 권당 1만전의 벌금을 물어야 한다네.", { prev = true, next = true })
        if b == DIALOG_RESULT.QUIT then
            return
        end
        if b == DIALOG_RESULT.PREV then
        ...
        if me:money() < 20000 then
            me:dialog(npc, "물어줄 돈이 없구만. 썩 물러나게.", { prev = false, next = false })
            return
        end
