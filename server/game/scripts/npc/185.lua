...
local function pyosin_story(me, npc)
    ...
    if q == nil then
        ::NPC_250_0002::
        btn = me:dialog(npc, '헉! 누...누...... 누구시오!', { prev = false, next = true })
        ...
        if sel == 3 then
            me:dialog(npc, '아니네. 허허. 그럼 잘가보게.', { prev = false, next = false })
            return
        end
        ::NPC_250_0004::
        ...
        ::NPC_250_0005::
        btn = me:dialog(npc, '이런 곳이라면 어디에 언제 귀신이 나타나도 이상하지 않지... 난 연구를 해야 하니 이 곳에 있을 수 밖에 없지만.', { prev = true, next = true })
        ...
        ::NPC_250_0006::
        btn = me:dialog(npc, '아, 이거 내 소개가 늦었군. 나는 저 표신궁에 얽힌 수수께끼의 역사들을 연구하는 역사가라오. 대장군이 왜 사람들을 죽였는가, 폐허가 되다시피한 표신궁 주변과는 달리, 표신궁은 어떻게 아직도 건재할 수 있는 것인가... 이런 것들을 연구하고 있지.', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0005 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        if sel == 3 then
            me:dialog(npc, '당신도 그렇게 생각하시오? 그럼 부디 몸조심하시게.', { prev = false, next = false })
            return
        end
        ::NPC_250_0008::
        ...
        ::NPC_250_0009::
        btn = me:dialog(npc, '대체 표신궁 깊은 곳에는 무엇이 있는지...... 아무튼 그래서 말인데, 당신이 표신궁에 들어간다면 내 부탁 하나만 들어주지 않겠소? 사례는 후하게 드리겠소이다.', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0008 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        if sel == 3 then
            me:dialog(npc, '하긴.. 보통 위험한 일이 아니니 다음에 기회가 되면 도와주시게.', { prev = false, next = false })
            return
        end
        ::NPC_250_0011::
    ...
    if q:completed() then
        me:dialog(npc, '요즘 어떻게 지내시나? 당신 덕분에 내 연구에 많은 진척이 있었소. 정말 고맙소.', { prev = false, next = false })
        return
    end
    ...
    if step == 1 then
        ...
        if sel == 3 then
            me:dialog(npc, '그렇군. 너무 서두르지는 않아도 되니 천천히 하시게나.', { prev = false, next = false })
            return
        end
        ...
        ::NPC_250_0016::
        btn = me:dialog(npc, '가까스로 살아나온 사람들에게서도 그런 이야기는 듣지 못했소이다. 하물며 동장군의 원령을 쓰러뜨리기까지 하다니! 오래간만에 대단한 모험가를 만난 것 같구려. 허허허...', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0015 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0017::
        btn = me:dialog(npc, '그럼 잠시만 기다려 보시오. 이 문서를 냉큼 해독해 보일 테니...', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0016 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0019::
        btn = me:dialog(npc, '그 대장군은 원래 선량한 사람이었으나 천하를 통일하고 권력을 잡은 후 수많은 사람을 학살한 폭군이 되고 말았다 하오. 권력이라는게 다 그런 법이긴 하지만...', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0018 
        end
        if btn == DIALOG_RESULT.QUIT then 
    ...
    if step == 2 then
        ...
        ::NPC_250_0022::
        btn = me:dialog(npc, '일기의 다음이 궁금해지는군. 혹시 표신궁에 또 갈 일이 있거든 이 일기의 다음편을 찾아봐 주셨으면 하오. 물론 사례는 하겠소.', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0021 
        end
        if btn == DIALOG_RESULT.QUIT then 
    ...
    if step == 3 then
        if not me:has_items(story_items[2].doc, 1) then
            me:dialog(npc, '아직 오방장군중 하나인 서장군을 물리치지 못하신것 같군. 서장군을 물리친 후 일기를 구해와주시오.', { prev = false, next = false })
            return
        end
        ::NPC_250_0024::
        ...
        ::NPC_250_0025::
        btn = me:dialog(npc, '......음...호오...... 과연, 과연... 그랬군. 허허허...', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0024 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        btn = me:dialog(npc, '다 읽고 나면 말씀해 주시오. 역시 또 부탁할 것이 있을 것 같구려. 허허...', { prev = false, next = false })
        return
    end
    ...
    if step == 5 then
        if not me:has_items(story_items[3].doc, 1) then
            me:dialog(npc, '아직 표신궁고문서3을 구하지 못했군. 남장군을 쓰러뜨리고 고문서를 얻어와주게.', { prev = false, next = false })
            return
        end
        ::NPC_250_0028::
        ...
        ::NPC_250_0029::
        btn = me:dialog(npc, '잠시만 기다려 주시오. 금방 번역해 보이겠소이다.', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0028 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0030::
        btn = me:dialog(npc, '......음...그런 일이... 그 때는 그랬었군, 과연... 으음!', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0029 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        btn = me:dialog(npc, '다 읽고 나면 말씀해 주시오. 음...', { prev = false, next = false })
        return
    end
    ...
    if step == 7 then
        if not me:has_items(story_items[4].doc, 1) then
            me:dialog(npc, '아직 표신궁고문서4를 구하지 못했군. 북장군을 쓰러뜨리고 고문서를 얻어와주게.', { prev = false, next = false })
            return
        end
        ::NPC_250_0033::
        ...
        ::NPC_250_0034::
        btn = me:dialog(npc, '......음...쯧쯧쯧...그랬군, 설마했더니...으음......', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0033 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        btn = me:dialog(npc, '다 읽고 나면 말씀해 주시오...', { prev = false, next = false })
        return
    end
    ...
    if step == 9 then
        if not me:has_items(story_items[5].doc, 1) then
            me:dialog(npc, '아직 표신궁고문서5를 구하지 못했군. 중앙장군을 쓰러뜨리고 고문서를 얻어와주게.', { prev = false, next = false })
            return
        end
        ::NPC_250_0037::
        ...
        ::NPC_250_0038::
        btn = me:dialog(npc, '...으음....음....!', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0037 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0039::
        btn = me:dialog(npc, '이런! ......호오...이런 일이 있었군......', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0038 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0040::
        btn = me:dialog(npc, '역시 그런 일이 있었기에... 그렇군, 과연...! 하지만 이게 정말일까? 으음...!!', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0039 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        q:step(10)
        btn = me:dialog(npc, '굉장히 흥미로운 내용이 쓰여있소. 하지만 너무 신비한 내용이라 믿어지지 않는구려. 어찌 되었건 한 번 읽어 보시오.', { prev = false, next = false })
        return
    end
    ...
    if step == 10 then
        ...
        ::NPC_250_0043::
        btn = me:dialog(npc, '하지만 유감스럽게도 그것이 사실인지를 파악할 수 있는 다음 장은 찢겨져 있더군.', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0042 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0044::
        btn = me:dialog(npc, '수고스럽겠지만 표신궁을 샅샅이 뒤져봐 주시오, 진실을 밝히기 위해선 대장군의 일기 다음 장... 그러니까 여섯번째의 표신궁고문서가 꼭 필요하지. 그것을 찾아와 주셨으면 하오.', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0043 
        end
        if btn == DIALOG_RESULT.QUIT then 
    ...
    if step == 11 then
        if not me:has_items(story_items[6].doc, 1) then
            me:dialog(npc, '여섯번째 고문서는 아마 표신궁 어딘가에 숨겨져 있는듯 하오. 구석구석 찾아봐 주시게.', { prev = false, next = false })
            return
        end
        ::NPC_250_0046::
        ...
        ::NPC_250_0047::
        btn = me:dialog(npc, '......으음......?', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0046 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0048::
        btn = me:dialog(npc, '음... 이런! ......호오...이럴 수가......', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0047 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0049::
        btn = me:dialog(npc, '음... 과연, 그렇군! 오오, 신이시여...어찌 이런 일이...!', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0048 
        end
        if btn == DIALOG_RESULT.QUIT then 
    ...
    if step == 12 then
        ...
        ::NPC_250_0053::
        btn = me:dialog(npc, '대장군이 천하통일 후 왜 흉악한 폭군이 되었는지... 왜 그렇게 죄없는 사람들을 학살했는지... 주인 없는 표신궁이 어떻게 아직도 건재할 수 있는지... 이제야 그 이유를 알 수 있게 됐구려...', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0052 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0055::
        btn = me:dialog(npc, '......혹시!?', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0054 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0056::
        btn = me:dialog(npc, '이...이런!!! 큰일이오. 어쩌면... 어쩌면... 내 예상이 맞다면 정말 큰일이 일어날 것이오. 시간이 없소. 다신이 가지고 있는 일기장을 전부 조합해 보시오! 어서!', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0055 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        if me:mkitem('대장군의일기겉표지', 1) then
            me:dialog(npc, '여기 책 겉표지도 있으니 같이 조합하시면 될 거요!', { prev = false, next = false })
        end
        return
    end
    ...
    if step == 13 then
        if not me:has_items('대장군의일기통합본', 1) then
            me:dialog(npc, '어서 조합해보시게. 겉표지와 같이 조합하면 될걸세!', { prev = false, next = false })
            return
        end
        ::NPC_250_0058::
        ...
        ::NPC_250_0059::
        btn = me:dialog(npc, '...내가 예전에 조사했던 고문서가 있소. 어떤 날짜와 알 수 없는 숫자들이 적혀 있는 문서였는데...', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0058 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0060::
        btn = me:dialog(npc, '이 일기들이 쓰여진 시기와 대조해 보니, 그것은... 대장군이 부활하게 될 날짜가 적힌 고문서였던 거요!', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0059 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        ::NPC_250_0063::
        btn = me:dialog(npc, '...아무튼 수고하셨소. 당신 덕분에 연구에 많은 진척이 있었소. 정말 고맙소. 이것은 이제껏 연구를 도와준 사례요. 재미있게 생긴 돌이지? 꽤 값어치가 나간다고 들었소만... 아마 오랜 경력을 가진 대장장이라면 이 돌을 단번에 알아볼 거요.', { prev = true, next = true })
        if btn == DIALOG_RESULT.PREV then 
            goto NPC_250_0062 
        end
        if btn == DIALOG_RESULT.QUIT then 
...
local function pyosin_sell_docs(me, npc)
    ...
    ::NPC_250_0066::
    btn = me:dialog(npc, '고문서는 권수가 높을수록 좀 더 높은가격으로 사고있네.', { prev = true, next = true })
    if btn == DIALOG_RESULT.PREV then
        goto NPC_250_0065
    end
    if btn == DIALOG_RESULT.QUIT then
