...
local function dojaeyoung_nobidocument(me, npc)
    ...
    local sel, lb = me:list(npc, '아니, 이것은... 얼마 전에 풀어 줬던 황비연의 노비문서군요. 생각납니다. 여기서 일하던 노비였는데, 참 의협심이 강한 청년이였죠. 그는 잘 있나요?', { '예, 실은...' }, { prev = true })
    if lb == DIALOG_RESULT.QUIT or sel ~= 1 then
        return
    end
    ::NPC_107_0001::
    local btn = me:dialog(npc, '예? 뭐라고요? 그게 사실입니까? 하하... 이거 참, 뭐라고 말해야 할지...\n\n황비연... 그가 의적이 되어 탐관오리를 혼내주고 사람들을 돕고 있단 말이지요?', { prev = false, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    ::NPC_107_0010::
    btn = me:dialog(npc, '뭐라고 해야 할까... 참 대견스럽군요. 그가 어느날 갑자기 어려운 사람들을 돕고 싶다며 풀어달라고 할 때에는 반신반의했었지요.', { prev = true, next = true })
    ...
    sel, lb = me:list(npc, '지금 보니 그 때 황비연을 저희 집안 노비 신세에서 풀어준 것이 잘한 일이었다는 생각이 드는군요.', { '.....' }, { prev = false })
    if lb == DIALOG_RESULT.QUIT or sel ~= 1 then
        return
    end
    ::NPC_107_0011::
    btn = me:dialog(npc, '이 보패를 그에게 전해주십시오. 이 문서를 제게 전해달라고 부탁했다는 그 사람이 아마 황비연 본인일 겁니다.', { prev = true, next = true })
    if btn == DIALOG_RESULT.PREV then
        goto NPC_107_0010
    end
    if btn == DIALOG_RESULT.QUIT then
...
local function dojaeyoung_herb_start(me, npc)
    ...
    ::NPC_107_0021::
    btn = me:dialog(npc, '아무래도 이런 마법이 걸린 곳에서 보통 사람이 사는것은 무리인지, 저희 아버지도 요즘 건강이 안좋으시죠.', { prev = true, next = true })
    if btn == DIALOG_RESULT.PREV then
        goto NPC_107_0020
    end
    if btn == DIALOG_RESULT.QUIT then
    ...
    ::NPC_107_0022::
    local sel, lb = me:list(npc, '의사의 말에 따르면 도삭산의 토끼들이 가지고 있는 약초를 달여서 지은 약을 드시면 차도가 있으실꺼라고 했는데...', { '제가 구해드리겠습니다.', '그렇군요..' }, { prev = false })
    ...
    ::NPC_107_0025::
    btn = me:dialog(npc, '아.. 정말 감사합니다. ' .. me:name() .. '님은 마음도 넓으신 분이시군요.', { prev = true, next = true })
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    ::NPC_107_0026::
