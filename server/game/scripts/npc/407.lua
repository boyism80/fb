...
return {
    ON_CLICK = function(me, npc)
        ...
        button = me:dialog(npc, prefix .. "비약을 만들기 위해선 " .. button_str .. " 2개가 필요하다네.", { prev = false, next = true })
        if button == DIALOG_RESULT.QUIT then
            return
        end

        button = me:dialog(npc, "흠흠.. 그리고 말일세. 자네 요즘 중국에서 유행 한다는 '검정깃발'을 알고 있나? 그거 하나만 가졌으면 좋겠는데 말이야..", { prev = false, next = true })
        if button == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if sel2 == 2 then
            me:dialog(npc, "비약을 만들고 싶으면 어서 구해오게.", { prev = false, next = false })
            return
        end
        ...
        if code == enum.EXCHANGE_RESULT.LACK_COST then
            if not me:has_items(button_str, 2) then
                me:dialog(npc, "아니 자네," .. button_str .. " 2개가 없지 않은가.", { prev = false, next = false })
            elseif not me:has_items("검정깃발", 1) then
                me:dialog(npc, "아니 자네, 검정깃발이 없지 않은가.", { prev = false, next = false })
            else
                me:dialog(npc, "아니 자네," .. button_str .. " 2개와 검정깃발이 없지 않은가.", { prev = false, next = false })
            end
            return
        end
        ...
        me:dialog(npc, "자. 여기 " .. reward_name .. " 일세. 받게나.", { prev = false, next = false })
    end
}
