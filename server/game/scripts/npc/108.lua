...
return {
    on_click = function(me, npc)
        ...
        if q == nil then
            me:dialog(npc, '쿨럭..쿨럭..', { prev = false, next = false })
            return
        end
        ...
        if q:step() < 2 then
            ...
            ::NPC_108_0002::
            btn = me:dialog(npc, '(안색이 별로 좋지 않다.)', { prev = true, next = false })
        ...
        if q:step() == 2 then
            ::NPC_108_0010::
            local btn = me:dialog(npc, '오.. ' .. me:name() .. '씨 아닌가. 아들놈에게 약초를 구해셨다고 하더군요. 덕분에 내가 이렇게 건강을 되찾았소. 고맙구려.', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_108_0011::
            ...
            if sel == 2 then
                me:dialog(npc, '허허.. 바쁘시다면 할 수 없군요.', { prev = false, next = false })
                return
            end
            if sel ~= 1 then
                return
            ...
            ::NPC_108_0015::
            btn = me:dialog(npc, '오오. 고맙소. 인어의방울..은 도삭산고양이인어가 가지고 있지 않을까? 고양이목에 방울달기라는 말도 있지 않소? 하하하. 농담이지만 말이지.', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_108_0016::
        ...
        if q:step() == 3 then
            ...
            ::NPC_108_0020::
            btn = me:dialog(npc, '오.. 이것이 인어의방울이군. 고맙소고맙소.', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_108_0021::
            ...
            if sel == 2 then
                me:dialog(npc, '그런가? 흐음..딸이 하도 칭얼대서 말이지..', { prev = false, next = false })
                return
            end
            if sel ~= 1 then
                return
        ...
        if q:step() == 4 then
            ...
            ::NPC_108_0030::
            btn = me:dialog(npc, '오,, 이것이 인어의거울이군! 고맙소!! 정말 고맙소!', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_108_0031::
            btn = me:dialog(npc, '이걸 어떻게 사례하나.. 그렇지. 우리 집안에 내려오는 목걸이를 하나 드리지.\n\n\'봉마의목걸이\'라고 해서.. 마귀를 봉인하여 담았다고 하는 옥으로 만든것이란느데.. 정말인지는 모르겠소.', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
            ...
            if code == enum.exchange_result.LACK_COST then
                me:dialog(npc, '인어의거울을 가지고 있지 않으시군요.', { prev = false, next = false })
                return
            end
            if code == enum.exchange_result.LACK_CAPACITY then
                me:dialog(npc, '소지품이 가득 차서 봉마의목걸이를 받을 수 없습니다.', { prev = false, next = false })
                return
            end
            q:step(5)
            me:push_achievement(9, '도삭산 200층 퀘스트 완료', 7, 1)
            me:dialog(npc, '뭐 어쨌든 기념으로 드릴테니.. 잘 쓰시오..\n\n그럼 건강하시오!', { prev = false, next = false })
            return
        end

        me:dialog(npc, me:name() .. '씨군, 요세 잘 지내나? 당신 덕분에 감사한일이 정말 많소.', { prev = false, next = false })
    end
}
