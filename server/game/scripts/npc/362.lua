...
function NPC_362(me, npc)
    ...
    if q:progress() >= 1 then
        ...
        button = me:dialog(npc, "전 대청소가 정말 싫어요. 발명할때 사용할 재료들은 잡동사니가 아니란 말이에요! 저에겐 꼭 필요한건데도 몽연이형은 다 치우라고 하고..", { prev = false, next = true })
        if button == DIALOG_RESULT.QUIT then
            return
        end
    ...
    if me:has_items("연", 1) and me:has_items("깨진김장독", 1) and me:has_items("찌그러진냄비", 1) and me:has_items("널", 1) and me:has_items("실패", 1) then
        ...
        button = me:dialog(npc, "연, 실패, 깨진김장독, 찌그러진냄비, 널을 가지고 계시군요! 그걸 저에게 주시면.. " .. name_with(reward_name, '을', '를') .. " 드릴 수 있는데...", { prev = false, next = true })
        if button == DIALOG_RESULT.QUIT then
            return
        end
