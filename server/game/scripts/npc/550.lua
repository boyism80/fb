...
local function run_form(me, npc, class_name, partner, schedule)
    ...
    local sel, btn = me:list(npc, "마동 최강자전 참가 신청입니다. 모든 공란을 채워 넣으신 후, 제출 버튼을 눌러주세요. 제출 뒤에는 수정이 불가능 하니 신중하게 검토하여 신청 하시기 바랍니다.", labels, false)
    ...
    if sel == 2 then
        ...
        local sub, sub_btn = me:list(npc, "참여하실 예선전의 일정을 선택 해 주세요.", sched_labels, false)
        if sub_btn == DIALOG_RESULT.QUIT then
            return
        end
...
function NPC_550(me, npc)
    ...
    if me:promotion() < 3 then
        me:dialog(npc, "3차승급 미만은 참여하실 수 없습니다.", false, false)
        return
    end

    local d = me:dialog(npc, "약 1주일간 참가신청을 받은 뒤, 참가 자격 필터링을 한 뒤 최종 참가자를 발표 합니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
    d = me:dialog(npc, "참가팀은 총 64팀이며, 자진기권 등의 가능성을 염두에 두어 예비 팀도 약 10팀을 선발합니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
    d = me:dialog(npc, "2인 1조로 참여하실 수 있으며, 함께 참가하실 파트너와 함께 참가 등록을 하셔야만 합니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
    d = me:dialog(npc, "참가 과정에서 별도의 파트너 동의 여부는 이루어지지 않습니다만, 추 후 검토하게 되므로 반드시 신중하게 참가 신청을 해 주시길 바랍니다.", false, true)
    if d == DIALOG_RESULT.QUIT then
        return
    end
