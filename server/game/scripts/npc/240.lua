...
function NPC_240(me, npc)
    ...
    if q == nil then
        ...
        ::NPC_240_0001::
        local btn = me:dialog(npc, '......음... 꼬마가 기력이 없고 말을 잘 하지 못한다... 그럴 때에는 삼전신보탕이 제격이지.', false, true)
        ...
        btn = me:dialog(npc, '재료를 가져오기만 하면 만드는 건 금방이라네. 그럼 힘 내게나.', false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
    ...
    if q:step() == 1 then
        ...
        btn = me:dialog(npc, '자, 자 되었네. 이걸 먹으면 그 애도 건강해질 거야. 허허. 수고했네.', false, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
