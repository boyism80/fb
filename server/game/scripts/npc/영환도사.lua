...
function NPC_542(me, npc)
    ...
    if step == 6 then
        ...
        if list_btn == DIALOG_RESULT.QUIT then return end
        if list_btn == DIALOG_RESULT.PREV then goto NPC_542_0003 end
        ...
        if button == DIALOG_RESULT.QUIT then return end
        if button == DIALOG_RESULT.PREV then goto NPC_542_0005 end

        ::NPC_542_0007::
        button = me:dialog(npc, "강시들을 물리쳐 강시의손톱을 모아와주게. 100개의 강시의 손톱을 모아와보도록 하게나. 만년원귀를 바로 상대했다가 개죽음을 당할 수도 있지 않는가.", true, true)
        if button == DIALOG_RESULT.QUIT then return end
        if button == DIALOG_RESULT.PREV then goto NPC_542_0006 end
    ...
    if step == 7 then
        ...
        if button == DIALOG_RESULT.QUIT then return end
        if button == DIALOG_RESULT.PREV then goto NPC_542_0008 end
    ...
    if button == DIALOG_RESULT.QUIT then return end
    if button == DIALOG_RESULT.PREV then goto NPC_542_0010 end

    ::NPC_542_0012::
    button = me:dialog(npc, "음.. 음.. 내가 가진 검과 비슷하게 생긴것인가보군?", true, true)
    if button == DIALOG_RESULT.QUIT then return end
    if button == DIALOG_RESULT.PREV then goto NPC_542_0011 end

    ::NPC_542_0013::
    button = me:dialog(npc, "엇. 이 칼의 귀퉁이에 조그맣게 글자가 써있는데... 아니 이것이 혈황검이었군! 등잔밑이 어둡다더니..", true, true)
    if button == DIALOG_RESULT.QUIT then return end
    if button == DIALOG_RESULT.PREV then goto NPC_542_0012 end

    ::NPC_542_0014::
    button = me:dialog(npc, "그럼 이걸 자네에게 주도록 하지. 나에게 필요한 물건도 아니니..", true, true)
    if button == DIALOG_RESULT.QUIT then return end
    if button == DIALOG_RESULT.PREV then goto NPC_542_0013 end
