local function pyosin_story(me, npc)
    ...
    if quest == nil then
        ::COS002::
        btn = me:dialog(npc, '헉! 누...누...... 누구시오!', true, true)
        ...
        ::COS012::
        btn = me:dialog(npc, '이것은 내 작은 성의요. 착수금조로 받아주시오.', true, true)
        if btn == DIALOG_RESULT.PREV then 
            goto COS011 
        end
        if btn == DIALOG_RESULT.QUIT then 
        ...
        if not me:start_quest(QUEST_PYOSIN) then
            me:dialog(npc, '퀘스트 시작 실패', false, true)
            return
        end
        quest = me:quest(QUEST_PYOSIN)
        quest:step(1)
...
local function pyosin_sell_docs(me, npc)
    ...
    if not me:has_items(doc, 1) then
        me:dialog(npc, doc .. '을 가지고 있지 않은데?', false, false)
        return
    end
    local confirm = me:list(npc, doc .. '을 ' .. price .. '전에 팔겠는가?', { '예, 팔겠습니다.', '아니오, 좀더 생각해볼게요.' })
    if confirm == nil then 
        return
    end
    if confirm ~= 0 then 
    ...
    me:rmitem(doc, 1, ITEM_DELETE_TYPE.GIVE)
