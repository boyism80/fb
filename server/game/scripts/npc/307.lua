...
return {
    on_click = function(me, npc)
        ...
        d = me:dialog(npc, "미궁에서 혼자 지내려니 무엇보다 술이 고프더군... 혹 작약주가 있다면 무기를 내어줌세...", { prev = false, next = true })
        if d == DIALOG_RESULT.QUIT then
            return
        end
        d = me:dialog(npc, "신작약주가 있다면, 전사 전용의 신무기까지 만들어 줄 수 있네만...", { prev = false, next = true })
        if d == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if sel == 1 then
            ...
            me:dialog(npc, "작약주를 가지고 있구만... 예전엔 참 좋은 술이었는데...\n\n여기 방천화극이 있네. 종종 들리게나...", { prev = false, next = false })
            return
        end
        ...
        if sel == 4 then
            ...
            if pn == 2 then
                ...
                if me:mkitem("용궁의보물", 1) == nil then
                    me:dialog(npc, "소지품이 가득 차서 용궁의보물을 드리지 못합니다.", { prev = false, next = false })
                    return
                end
                if q == nil then
                ...
                me:dialog(npc, "옛소. 이걸 받는 대신 내가 가져갔다는 말 하면 안되오.", { prev = false, next = false })
                return
            end
            if pn >= 3 then
                me:dialog(npc, "아무한테도 말하지 말아주시오.", { prev = false, next = false })
                return
            end
            me:dialog(npc, "그건 왜 찾는가?", { prev = false, next = false })
        end
    end
}
