...
local function run_ox_admin(me, ch)
    if not me:has_items('아무나OX주최토큰', 1) then
        return me:dialog(ch, '아무나OX주최토큰이 없으시군요.', false, true)
    end
    ...
    elseif sub_sel == 2 then
        ...
        m:block(7, 13, true)
        m:block(21, 13, true)
        return me:dialog(ch, '출입구를 닫았습니다.', false, true)
    end

    local raw = me:input(ch, '강제퇴장할 유저의 아이디를 적어주세요.', '퇴장할 대상의 아이디는:', '입니다.', 6, false)
    if raw == DIALOG_RESULT.QUIT then
        return nil
    end
    if raw == nil or raw == '' then
        return me:dialog(ch, '대상 아이디를 입력해 주세요.', false, true)
    end

    local target = name2ch(raw)
    if target == nil then
        return me:dialog(ch, '해당 유저가 접속중이 아닙니다.', false, true)
    end
    if me:map() ~= target:map() then
        return me:dialog(ch, '그럴 수 없어 뿍짝아...', false, true)
    end

    local btn = me:dialog(ch, raw .. '님이 강제 퇴장 됩니다..', false, true)
        ...
        menu = {
            ...
            { '토큰 구입', function(me, ch)
                ...
                if code == enum.EXCHANGE_RESULT.LACK_COST then
                    return me:dialog(ch, '도깨비방망이가 없으시군요.', false, true)
                end
                if code == enum.EXCHANGE_RESULT.LACK_CAPACITY then
                    return me:dialog(ch, '소지품이 가득 차서 아무나OX주최토큰을 받을 수 없습니다.', false, true)
                end
                return me:dialog(ch, "도깨비방망이를 '아무나OX주최토큰'으로 바꿔드리겠습니다.", false, true)
            end },
        },
    })
end