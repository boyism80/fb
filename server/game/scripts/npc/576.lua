...
return {
    on_click = function(me, npc)
        ...
        local btn = me:dialog(npc, "안녕하세요? 저는 [황금도토리]를 모으고 있는 [토로라비]입니다. 톨루이와는 오누이 지간입니다.", { prev = false, next = true })
        ...
        if sel == 1 then
            ...
            me:dialog(npc, "황금도토리를 일반교환권으로 교환해 드렸어요.", { prev = false, next = false })
            return
        end
        ...
        if sel == 2 then
            ...
            me:dialog(npc, "황금도토리를 특별교환권으로 교환해 드렸어요.", { prev = false, next = false })
            return
        end

        if sel == 3 then
            me:dialog(npc, "저는 세시간에 한번 찾아온답니다. 제가 사라지면, 세시간 뒤에 저를 다시 찾아주세요.", { prev = false, next = false })
        end
    end
}
