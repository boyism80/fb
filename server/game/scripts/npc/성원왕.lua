...
local function do_sub6_start(me, npc)
    ...
    local b = me:dialog(npc, "이 땅에는 호박이라는 아주 예쁜 물건이 있으시다고 들으셨다. 병에 걸리신 원숭이들 때문에", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
    b = me:dialog(npc, "마음에 상처를 입으신 우리 원숭이들을 위로해주실 수 있으실지도 모르신다. 그걸 자네가 좀", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
    b = me:dialog(npc, "구해주시길 원하신다. 그래 한 80개면 충분하실거다.", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
...
local function do_sub6_turnin(me, npc)
    ...
    local b = me:dialog(npc, "굉장히 예쁜 물건이시다! 호박이라는게 이렇게 예쁘실 줄이야! 모두에게 나누어드리면 시름을 잊으시고", false, true)
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    b = me:dialog(npc, "들여다 보실거다! 고마우시다! 아주 수고하셨다! 이건 내가 몰래 숨겨가지고 계시던 간식이시다.", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
...
function NPC_468(me, npc)
    ...
    if main_q then
        local s = main_q:step()
        if s == 20 then
            local b = me:dialog(npc, "참원왕이 의견을 물어 여기까지 왔으시라고?", false, true)
            if b == DIALOG_RESULT.QUIT then
                return
            end
            b = me:dialog(npc, "인간들은 음식을 구워서 드신다며? 잘은 모르시지만, 우리도 음식을 구워 드시면 병에 걸리시는 일이", true, true)
            if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
                return
            end
            b = me:dialog(npc, "없으실 것만 같으시다. 그 횃불이라고 부르시는 물건을 구하셔서 구워 드셔보는게 어떠실까 하신다.", true, true)
            if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
                return
            end
            main_q:step(21)
        ...
        if s == 21 then
            me:dialog(npc, "내 의견을 참원왕에게 어서 전달해주셨으면 좋으시겠다.", false, false)
            return
        end
        if s == 30 then
