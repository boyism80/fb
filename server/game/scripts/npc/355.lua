...
return {
    on_click = function(me, npc)
        ...
        if sel == 1 then
            local btn = me:dialog(npc, "음력 5월 5일은 단오에요.", { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
        ...
        if sel == 2 then
            local btn = me:dialog(npc, "창포를 구해오셨나요?", { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
        ...
        if sel == 3 then
            local btn = me:dialog(npc, "창포뿌리를 다섯개나 모으셨어요? 그럼 제가 비녀를 만들어 드릴께요.", { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
