...
return {
    ON_CLICK = function(me, npc)
        ...
        if q == nil then
            ::NPC_183_0001::
            local btn = me:dialog(npc, '아.. 이 근처에 있다고 했는데... 어디있는 것일까...', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            ::NPC_183_0002::
            btn = me:dialog(npc, '아, 반갑네 내가 지금 뭘 좀 찾고 있느라 말이지. 실은 아내가 아이를 가졌는데 화기삼동초돈유합을 너무 먹고 싶다고 하는구만.', { prev = true, next = true })
            ...
            ::NPC_183_0003::
            btn = me:dialog(npc, '그래서 여기저기서 전해지는 요리법으로 내가 한번 만들어보려고 하는데 재료조차 구하기가 어렵군.', { prev = true, next = true })
            if btn == DIALOG_RESULT.PREV then
                goto NPC_183_0002
            end
            if btn == DIALOG_RESULT.QUIT then
        ...
        if not me:has_items('화기삼동충초돈유합', 1) then
            ::NPC_183_0005::
            btn = me:dialog(npc, '아직 화기삼동충초돈유합을 구하지 못하셨나보군요.', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
        ...
        btn = me:dialog(npc, '이걸 먹으면 자네의 체력이 더 강해질 걸세! 그럼 고맙네.. 잘가게나.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
