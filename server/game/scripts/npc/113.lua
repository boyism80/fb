...
return {
    ON_CLICK = function(me, npc)
        ...
        if q == nil then
            ...
            ::NPC_113_0005::
            btn = me:dialog(npc, '그대는 비록 외부인이지만 정령의 힘을 이해했다... 노력에 대한 보답으로... 다음 재료가 무엇인지 가르쳐준다...', { prev = true, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            if btn == DIALOG_RESULT.PREV then
        ...
        if q:step() == 2 then
            if not me:has_items('자연의인장', 1) then
                me:dialog(npc, '그대는 아직 자연의인장을 구하지 못했다..', { prev = false, next = false })
                return
            end
