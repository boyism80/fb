function NPC_183(me, npc)
    ...
    if quest == nil then
        ::NPC_183_COS001::
        local btn = me:dialog(npc, '아.. 이 근처에 있다고 했는데... 어디있는 것일까...', true, true)
        ...
        ::NPC_183_COS004::
        local sel = me:list(npc, '날 좀 도와주지 않겠는가?', { '물론입니다. 부인의 몸조리에나 신경쓰십시오.', '죄송합니다만 그런일까지 할 시간이 없군요.' })
    ...
    if not me:has_items('화기삼동충초돈유합', 1) then
        ::NPC_183_COS005::
        btn = me:dialog(npc, '아직 화기삼동충초돈유합을 구하지 못하셨나보군요.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
    ...
    btn = me:dialog(npc, '이걸 먹으면 자네의 체력이 더 강해질 걸세! 그럼 고맙네.. 잘가게나.', true, true)
    if btn == DIALOG_RESULT.PREV then
        goto NPC_183_COS007
    end
    if btn == DIALOG_RESULT.QUIT then
