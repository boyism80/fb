...
local function do_sub9_receive_wine(me, npc)
    ...
    local b = me:dialog(npc, "음? 목원왕의 선물이시라고? 이런이런 생각도 못하셨었다. 이런 좋지 않으실때 위로를 해주시는건 역시 친구밖에 안계신다.", false, true)
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    b = me:dialog(npc, "고마우시다. 잘 받으셨다고 내 인사를 전해주셔야 좋으시다. 이정도면 우리 부족 원숭이들이 한모금씩 맛을 보실 수 있으시다.", true, true)
...
local function do_sub7_start(me, npc)
    ...
    local b = me:dialog(npc, "우리 부족의 아이가 한 명 쓰러지셨다. 다행히 흉폭해지시는 병은 아니시고, 단순한 열병이시다.", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
    b = me:dialog(npc, "나로서는 이 병을 치료해주실 능력이 없으시다. 우리 원숭이의 여러 부족들 중에 단 한 부족의", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
    b = me:dialog(npc, "우두머리만이 열병을 치료하시는 능력을 가지고 계시다. 그 우두머리를 찾으셔서 아이를 치료하시는게 좋으시다.", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
    me:mkitem("아픈아기원숭이", 1)
    ...
    if q7 == nil then
        q7 = me:start_quest(QUEST_SKULL_NECKLACE_7)
        if q7 == nil then
            return true
        end
    end
...
local function do_sub7_complete(me, npc)
    ...
    local b = me:dialog(npc, "하하하. 과연 건강하게 돌아오실 줄 알고 계셨다. 정말 수고하셨다! 그래, 이 아이의 웃음을 보고 계시면", false, true)
    if b == DIALOG_RESULT.QUIT then
        return false
    end
    b = me:dialog(npc, "참, 이건 작은 답례이시다. 이 근처에서 얻으신 물건이시다. 어디다 쓰시는지 도통 아시다가도 모르겠지만 말이시다.", true, true)
    if b == DIALOG_RESULT.PREV or b == DIALOG_RESULT.QUIT then
        return false
    end
...
function NPC_469(me, npc)
    ...
    if q7:step() == 1 then
        me:dialog(npc, "한 우두머리만이 원숭이를 치료할 수 있는 능력을 가지고 있으시다.", false, false)
        return
    end
    ...
    if main_q then
        ...
        if s == 24 then
            me:dialog(npc, "이런 의견을 빨리 참원왕에게 전해주시면 좋으시다.", false, false)
            return
        end
        if s == 26 then
