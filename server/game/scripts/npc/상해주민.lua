function NPC_232(me, npc)
    ...
    if quest == nil then
        me:dialog(npc, '안녕하신가요?', true, true)
        return
    end
    ...
    if quest:step() == 1 then
        local sel, lb = me:list(npc, '안녕하십니까. 허허, 아까부터 뭔가 찾으시는 눈치신데... 뭘 그리 찾으시는지요?', { '황비연이라는 사람을 찾고 있습니다.', '아무 것도 아닙니다.' }, true)
        if lb == DIALOG_RESULT.QUIT or sel ~= 0 then
            return
        end
        local btn = me:dialog(npc, '황비연? ...음, 황비연이라면 요즘 탐관오리의 재물을 훔쳐 가난한 사람들에게 나눠 준다는 의적 아닙니까?', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        sel, lb = me:list(npc, '참으로 훌륭한 사람이지요. 그런데 그는 왜 찾으십니까?', { '그를 잡아서 현상금을 타려고 합니다.', '꼭 한 번 만나 보고 싶은 사람입니다.' }, true)
        if lb == DIALOG_RESULT.QUIT then
            return
        end
        if sel == 0 then
            me:dialog(npc, '황비연이 현상금이 높긴 하죠... 하지만 쉽게 잡기는 어려울 겁니다.', true, true)
            return
        end
        if sel ~= 1 then
            return
    ...
    if quest:step() == 2 then
        me:dialog(npc, '아직 도삭산 100층에 계신분을 만나지 못하셨나 보군요.', true, true)
        return
    end
    ...
    if quest:step() == 3 then
        ...
        local sel, lb = me:list(npc, '문서는 전해 주고 오셨는지요?', { '예, 확실히 전해 줬습니다.', '아니오, 아직...' }, true)
        if lb == DIALOG_RESULT.QUIT or sel ~= 0 then
            return
        end
        local btn = me:dialog(npc, '그렇군요. 감사합니다. 수고하셨습니다... 그런데 이 보패는 웬 것입니까?', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ...
        btn = me:dialog(npc, '...다 알고 계셨군요. 하하. 이거 들켜버렸네요. 죄송합니다. 예, 제가 황비연입니다.\n\n부탁도 들어 주셨고, 제 할 일도 다 끝났습니다. 많은 사람을 도왔고, 나으리께서도 저를 자랑스럽게 여겨 주신다니 여한이 없습니다.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
