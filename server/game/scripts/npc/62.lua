...
return {
    ON_CLICK = function(me, npc)
        ...
        if q == nil then
            local btn = me:dialog(npc, '물병이 필요하신가요? 물병의 가격은 15만전입니다.', { prev = false, next = true })
            if btn == DIALOG_RESULT.QUIT then
                return
            end
            local sel = me:list(npc, '물병을 구입하시겠습니까?', { '예', '아니오' }, { prev = false })
        ...
        local btn = me:dialog(npc, '물병을 찾으러 오셨군요.', { prev = false, next = true })
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        ...
        if code == enum.EXCHANGE_RESULT.LACK_COST then
            if not me:has_items(VOUCHER_NAME, 1) then
                me:dialog(npc, '물병보관증을 가져오세요.', { prev = false, next = false })
            else
                me:dialog(npc, '15만전을 구해오세요.', { prev = false, next = false })
            end
            return
        end
        if code == enum.EXCHANGE_RESULT.LACK_CAPACITY then
            me:dialog(npc, '소지품이 가득 차서 물병을 받을 수 없습니다.', { prev = false, next = false })
            return
        end
        q:complete()
