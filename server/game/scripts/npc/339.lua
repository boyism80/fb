...
function NPC_339(me, npc)
    ...
    local btn = me:dialog(npc, "안녕하세요? 저는 배심원으로 참여한 여러분의 투표를 도와드릴 배심원투표도우미라고 합니다.", false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    btn = me:dialog(npc, "배심원 투표는 사적인 친분이나 이해관계에 따라 이뤄지면 안되며, 반드시 공정한 저울의 눈금 위에 있어야만 합니다.", false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    btn = me:dialog(npc, "따라서 모든 배심원 분들은 다음 선서를 읽고 이해하시기 바랍니다.", false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    btn = me:dialog(npc, "선서 첫번째", false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    btn = me:dialog(npc, "사실을 정당하게 판단할 것과 재판장이 설명하는 법과 증거에 의하여 진실하게 평결할 것을 엄숙히 선서합니다.", false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    btn = me:dialog(npc, "선서 두번째", false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    btn = me:dialog(npc, "모든 사실을 공정하고 정당하게 판결할 것이며, 만약 그러할 자신이 없다면 기권할 것을 엄숙히 선서합니다.", false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    ...
    btn = me:dialog(npc, "좋습니다. 그렇다면 이제 투표 절차를 시작하겠습니다.", false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
    btn = me:dialog(npc, "사적 친분이나 이해관계에 의한 투표가 아닌, 공정하고 진실하게 평결해주시기 바랍니다.", false, true)
    if btn == DIALOG_RESULT.QUIT then
        return
    end
