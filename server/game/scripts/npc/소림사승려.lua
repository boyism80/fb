function NPC_176(me, npc)
    ...
    if quest:step() == 1 then
        ::NPC_176_COS001::
        btn = me:dialog(npc, '안녕하십니까. 이런 곳에서 이국의 행인을 만나게 되다니, 이것도 다 인연이겠지요.', true, true)
        if btn == DIALOG_RESULT.QUIT then
            return
        end
        local sel = me:list(npc, '저에게 무슨 볼일이라도 있으신건지요?', { '소림사의 절기를 전수해주십시오.', '스님을 보니 반가워서 그만..' })
        if sel == nil then
            return
        end
        if sel == 1 then
        ...
        local sel2 = me:list(npc, '불도에 입문하시겠습니까?', { '네, 입문하겠습니다.', '아니오, 그건좀...' })
        if sel2 == nil then
            return
        end
        if sel2 == 0 then
        ...
        local sel3 = me:list(npc, '비단을 구해주실 수 있으시겠습니까?', { '네, 비단을 구해드리겠습니다.', '지금은 고구려로 돌아가기가 좀 곤란해서...' })
        if sel3 == nil then
            return
        end
        if sel3 == 1 then
    ...
    ::NPC_176_COS014::
    btn = me:dialog(npc, '그럼 또 인연이 닿으면 만나겠지요. 살펴가십시오. 나무아미타불..', true, true)
    if btn == DIALOG_RESULT.PREV then
        goto NPC_176_COS013
    end
    me:mkspell('백열장')
