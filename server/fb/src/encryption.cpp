...
// clang-format off
uint32_t encryption::HEX_TABLE[10][0x100] = 
{
...
encryption::encryption() :
    encryption(0, (uint8_t*)"NexonInc.")
{ }

encryption::encryption(uint8_t pattern, const uint8_t* iv) :
    _pattern(pattern),
    _iv(std::make_unique<uint8_t[]>(KEY_SIZE * 4))
{
    for (int i = 0; i < 4; i++)
    {
        std::memcpy(this->_iv.get() + KEY_SIZE * i, iv, KEY_SIZE);
    }
}
...
uint32_t encryption::encrypt(stream& data, uint32_t offset, uint32_t size)
{
    ...
    if (size > 1)
    {
        ...
        this->crypt(buffer_dst.get() + 2,
                    buffer_dst.get() + 2,
                    size - 1,
                    ((const uint8_t*)HEX_TABLE[this->_pattern]) + this->_sequence * 4,
                    1);
    }
...
uint32_t encryption::encrypt(stream& data)
{
    if (data.size() == 0)
        return -1;
...
uint32_t encryption::decrypt(stream& data)
{
    if (data.size() == 0)
        return -1;
