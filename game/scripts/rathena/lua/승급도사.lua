...
RESET:
    ...
    {
        if(@myjob == 4 && @myjoblv < 3)
        {
            setarray @hp[0], 25000, 80000, 250000, 600000;
            setarray @mp[0], 25000, 70000, 250000, 750000;

            if(get_level(@sd) >= 99 && get_purehp(@sd) >= @hp[@myjoblv] && get_puremp(@sd) >= @mp[@myjoblv]) // 승급기준인경우
            {
    ...
    else if(@ret == 2)
    {
        if(@myjob == 4)
        {
            ...
            if(@ret == 3 && @myjoblv >= 3)
            {
                setarray @magic$[0], "백호의희원'첨", "신령지익", "파력무참", "환군마술";
                set @ret, list1("안녕하세요. 어떤 기술을 배울래요?", @magic$[0], @magic$[1], @magic$[2], @magic$[3]);
                if(@ret == 0) { end; }
                set @pos, dialog(0, 0, 1, @magic$[@ret-1] + "을 배우기 위해선 5000만의 경험치가 필요합니다.");
                if(@pos == 0) { deletearray @magic$[0], getarraysize(@magic$); end; }
            ...
            if(@ret == 4 && @myjoblv >= 4)
            {
                setarray @magic$[0], "신령의기원'첨", "봉황의기원", "귀염추혼소";
