...
RESET:
    set @pos, list2(@ret, 0, 0, "안녕하세요. 어떻게 오셨나요?", @jobname$ + "으로서 승급을 원합니다", "기술을 배울래요", "3차승급 옷을 원합니다");
    if(@pos == 0) { end; }
    else if(@ret == 1)
    {
        if(@myjob == 2 && @myjoblv < 3)
        {
            setarray @hp[0], 65000, 220000, 600000, 1400000;
    ...
    else if(@ret == 2)
    {
        if(@myjob == 2)
        {
            ...
            if(@ret == 4 && @myjoblv >= 4)
            {
                setarray @magic$[0], "분혼경천", "파천검무", "개혈체식";
                set @ret, list1("안녕하세요. 어떤 기술을 배울래요?", @magic$[0], @magic$[1], @magic$[2]);
                if(@ret == 0) { end; }
                set @pos, dialog(0, 0, 1, @magic$[@ret-1] + "을 배우기 위해선 5000만의 경험치가 필요합니다.");
                if(@pos == 0) { deletearray @magic$[0], getarraysize(@magic$); end; }
