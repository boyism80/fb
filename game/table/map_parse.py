...
def extract_maps():
    ...
    for host, values in maps.items():
        with open(f'map.{host}.json', 'w', encoding='utf8') as f:
...
def extract_warps(maps):
    ...
    for host in warps:
        with open(f'warp.{host}.json', 'w', encoding='utf8') as f:
...
def extract_npc_spawns(maps):
    ...
    for host in spawns:
        with open(f'npc.spawn.{host}.json', 'w', encoding='utf8') as f:
...
def extract_mob_spawns(maps):
    ...
    for host in spawns:
        with open(f'mob.spawn.{host}.json', 'w', encoding='utf8') as f:
            f.write(json.dumps(spawns[host], indent=4, ensure_ascii=False))
