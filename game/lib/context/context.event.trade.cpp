...
{
    this->send(me, fb_resp::trade_close(TEXT(MESSAGE_TRADE_CANCELLED_BY_ME)), scope::SELF);
    this->send(you, fb_resp::trade_close(TEXT(MESSAGE_TRADE_CANCELLED_BY_PARTNER)), scope::SELF);
}

void context::on_trade_lock(character& me, character& you)
{
    this->send(me, fb_resp::trade_lock(), scope::SELF);
    this->send(you, fb_resp::message(TEXT(MESSAGE_TRADE_NOTIFY_LOCK_TO_PARTNER), MESSAGE_TYPE::POPUP), scope::SELF);
}

void context::on_trade_failed(character& me, character& you)
{
    this->send(me, fb_resp::trade_close(TEXT(MESSAGE_TRADE_FAILED)), scope::SELF);
    this->send(you, fb_resp::trade_close(TEXT(MESSAGE_TRADE_FAILED)), scope::SELF);
}

void context::on_trade_success(character& me, character& you)
{
    this->send(me, fb_resp::trade_close(TEXT(MESSAGE_TRADE_SUCCESS)), scope::SELF);
    me.update(STATE_LEVEL::EXP_MONEY);

    this->send(you, fb_resp::trade_close(TEXT(MESSAGE_TRADE_SUCCESS)), scope::SELF);
