...
|-------|----------|---------|
| `bind_default` | `controller.bind_default<T>()` | no-op |
| `clone` | copy from received header | `nullptr` |
| `create` | `nullptr` | `make_shared<T>()` |
| `marshal_lua` | field codegen | stub |

`protocol_registry::register_all()` registers response `clone` handlers by type key at `game_bot_controller` startup. `protocol_registry::bind_default()` binds default no-op handlers for response types that do not already have a handler.
