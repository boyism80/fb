...
public:
	...
	public:
		...
		_drop(const fb::game::item* item, float percentage) : item(item), percentage(percentage) {}
...
public:
	mob(const std::string& name, uint16_t look, uint8_t color = 0);
	mob(const mob& right);
	~mob();
...
public:
	...
	void					dropitem_add(const fb::game::item* item, float percentage);
