...
public:
    ...
    Quest(const Quest& x)
        : uid(x.uid), qid(x.qid), step(x.step), progress(x.progress), completed(x.completed)
    { }

    Quest(uint32_t uid, uint32_t qid, uint32_t step, uint32_t progress, bool completed)
        : uid(uid), qid(qid), step(step), progress(progress), completed(completed)
    { }

    Quest(const fb::protocol::internal::raw::Quest& raw)
        : uid(raw.uid()), qid(raw.qid()), step(raw.step()), progress(raw.progress()), completed(raw.completed())
    { }
