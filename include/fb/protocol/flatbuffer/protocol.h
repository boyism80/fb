 ...
 *          Generation timestamp: 26 Nov 2025
 */
...
public:
    ...
    StorageBox(const StorageBox& x)
        : user(x.user), id(x.id), message(x.message), attachments(x.attachments), received(x.received), expired_date(x.expired_date)
    { }

    StorageBox(uint32_t user, uint32_t id, const std::string& message, const std::string& attachments, bool received, const std::optional<std::string>& expired_date)
        : user(user), id(id), message(message), attachments(attachments), received(received), expired_date(expired_date)
    { }

    StorageBox(const fb::protocol::internal::raw::StorageBox& raw)
        : user(raw.user()), id(raw.id()), message(flatbuffers::option::decode(raw.message()->c_str())), attachments(flatbuffers::option::decode(raw.attachments()->c_str())), received(raw.received()), expired_date(raw.expired_date() != nullptr ? flatbuffers::option::decode(raw.expired_date()->c_str()) : std::optional<std::string>())
    { }
...
public:
    ...
    StoragePendingBox(const StoragePendingBox& x)
        : id(x.id), user(x.user), message(x.message), attachments(x.attachments), expired_date(x.expired_date)
    { }

    StoragePendingBox(unsigned long id, const std::optional<uint32_t>& user, const std::string& message, const std::string& attachments, const std::optional<std::string>& expired_date)
        : id(id), user(user), message(message), attachments(attachments), expired_date(expired_date)
    { }

    StoragePendingBox(const fb::protocol::internal::raw::StoragePendingBox& raw)
        : id(raw.id()), user(raw.user() != nullptr ? raw.user()->value() : std::optional<uint32_t>()), message(flatbuffers::option::decode(raw.message()->c_str())), attachments(flatbuffers::option::decode(raw.attachments()->c_str())), expired_date(raw.expired_date() != nullptr ? flatbuffers::option::decode(raw.expired_date()->c_str()) : std::optional<std::string>())
    { }
