...
public:
    ...
    Listing(const std::string& id, uint32_t seller_id, const fb::protocol::marketplace::Item& item, uint32_t price, uint32_t listing_fee, uint32_t transaction_fee, const std::string& expire_date, const std::string& created_date)
    ...
    Listing(const fb::protocol::marketplace::raw::Listing& raw)
        : id(flatbuffers::option::decode(raw.id()->c_str())), seller_id(raw.seller_id()), item(*raw.item()), price(raw.price()), listing_fee(raw.listing_fee()), transaction_fee(raw.transaction_fee()), expire_date(flatbuffers::option::decode(raw.expire_date()->c_str())), created_date(flatbuffers::option::decode(raw.created_date()->c_str()))
    { }
...
public:
    ...
    List(const List& x)
        : listing_id(x.listing_id), listing_fee(x.listing_fee), error(x.error)
    { }

    List(const std::string& listing_id, uint32_t listing_fee, uint32_t error)
        : listing_id(listing_id), listing_fee(listing_fee), error(error)
    { }

    List(const fb::protocol::marketplace::response::raw::List& raw)
        : listing_id(flatbuffers::option::decode(raw.listing_id()->c_str())), listing_fee(raw.listing_fee()), error(raw.error())
    { }
...
public:
    ...
    CheckListingStatus(const std::string& listing_id, uint32_t listing_fee, uint32_t error)
        : listing_id(listing_id), listing_fee(listing_fee), error(error)
    { }
