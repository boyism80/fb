...
private:
    ...
    using mob_ptr           = std::shared_ptr<fb::game::mob>;
    using mob_vector_t      = std::vector<mob_ptr>;
    using achievement_map_t = std::map<uint32_t, std::unique_ptr<achievement>>;
    using item_ptr          = std::shared_ptr<fb::game::item>;
    using quest_ptr         = std::shared_ptr<fb::game::quest>;
    using map_ptr           = std::shared_ptr<fb::game::map>;
    using object_ptr        = std::shared_ptr<fb::game::object>;
...
public:
    ...
    [[nodiscard]] async::task<bool>   map(map_ptr map, const fb::model::point16_t& position, DESTROY_TYPE destroy_type = DESTROY_TYPE::DEFAULT, bool notify = true) override final;
    void                              init();
    bool                              inited() const;
    ...
    uint32_t                          damage(uint32_t value, object_ptr from = nullptr, bool critical = false) override final;
    ...
    void                              sex(SEX value);
    STATE                             state() const;
    STATE                             state_to(const object& to) const;
    ...
    void                              item_tooltip(const item& item, uint16_t position);
    void                              show_user_list();
    void                              show_bulletin();
    ...
    mob_ptr                           spawn_mob(const fb::model::mob& model, const fb::model::point16_t& position, bool owned = true, bool notify = true);
    const mob_vector_t&               spawned_mobs() const;
    bool                              detach_spawned_mob(fb::game::mob& mob);
    async::task<void>                 death_penalty();
    quest_ptr                         quest(uint32_t id) const;
...
private:
    ...
    using character_function_t       = std::function<void(character_ptr&)>;
    using character_predicate_t      = std::function<bool(const character_ptr&)>;
    using character_async_function_t = std::function<async::task<void>(character_ptr&)>;
    using string_function_t          = std::function<void(const std::string&)>;

    std::unordered_map<uint32_t, character_ptr>    _from_uid;
    std::unordered_map<std::string, character_ptr> _from_name;
...
public:
    void          insert(character_ptr ch);
    void          remove(character_ptr ch);
    character_ptr find(uint32_t uid) const;
    bool          contains(const std::string& name) const;
    bool          contains(uint32_t uid) const;
    character_ptr find(const std::string& name) const;
    async::task<void> foreach (character_function_t&& fn, character_predicate_t predicate = nullptr);
    async::task<void> foreach (character_function_t&& fn, const std::vector<character_ptr>& characters);
    async::task<void> foreach_async(character_async_function_t&& fn, character_predicate_t predict = nullptr);
    async::task<void> foreach_async(character_async_function_t&& fn, const std::vector<character_ptr>& characters);
    async::task<void> foreach (const std::vector<std::string>& names, character_function_t && fn, string_function_t miss = nullptr);
    async::task<void> foreach_async(const std::vector<std::string>& names, character_async_function_t&& fn, string_function_t miss = nullptr);
    async::task<void> invoke(const std::string& name, character_function_t&& fn, string_function_t miss = nullptr);
    async::task<void> invoke_async(const std::string& name, character_async_function_t&& fn, string_function_t miss = nullptr);

public:
    character_ptr& operator[] (uint32_t uid);
    character_ptr& operator[] (const std::string& name);
};
