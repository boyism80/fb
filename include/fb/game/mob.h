...
public:
    struct initial_params : life::initial_params
    {
    public:
...
private:
    std::weak_ptr<life>             find_target();
    [[nodiscard]] async::task<bool> call_script();
    void                            AI(const fb::model::datetime& now);

public:
    bool     near_target(const std::shared_ptr<life>& target, DIRECTION& out) const;
...
public:
    ...
    std::shared_ptr<life>                     target() const;
    void                                      target(std::shared_ptr<life> value);
    std::shared_ptr<life>                     oblivion() const;
    void                                      oblivion(std::shared_ptr<life> value);
    std::shared_ptr<life>                     update_target();
    ...
    bool                                      push_item(std::shared_ptr<item> item);
    bool                                      hidden(const fb::game::object& target) const override final;
    void                                      hidden(bool enabled);
};

struct mob::listener_t : public virtual life::listener_t
{ };
