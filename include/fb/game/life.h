...
protected:
    async::task<mob_vector> damage_targets(const damage_list& targets, const damage_opts& opts);
...
public:
    ...
    virtual async::task<void> update(UPDATE_STATE_LEVEL value = UPDATE_STATE_LEVEL::EXP_MONEY | UPDATE_STATE_LEVEL::CROWD_CONTROL);
    async::task<void>         update_hp(uint32_t diff, bool critical, bool notify = true);
    batch_update_guard        batch_update();
    virtual async::task<void> kill(DESTROY_TYPE destroy_type = DESTROY_TYPE::DEFAULT);
    ...
    virtual async::task<void> action(ACTION action, DURATION duration, uint8_t sound = 0x00);
...
{
    // clang-format off
    virtual async::task<void> on_action(life& me, ACTION action, DURATION duration, uint8_t sound) = 0;
    virtual async::task<void> on_attack(life& me, DURATION duration = DURATION::ATTACK) = 0;
    virtual async::task<void> on_dead(life& me, std::shared_ptr<fb::game::object> you) = 0;
    virtual async::task<void> on_update_hp(life& me, uint32_t diff, bool critical) = 0;
    // clang-format on
};
 ...
 * Call commit() to flush the accumulated update as an awaitable send.
 *
 * Usage:
 * @code
 * auto batch = character.batch_update();
 * co_await character.stat.base_str(10, false);  // no packet
 * co_await character.stat.hp(500, false);       // no packet
 * co_await character.stat.mp(300, false);       // no packet
 * co_await batch.commit();                     // single combined packet sent here
 * @endcode
 */
