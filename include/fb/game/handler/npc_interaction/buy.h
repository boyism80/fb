...
/**
 * @brief   Handles NPC item purchasing interactions.
 *
 *          Processes player messages for item purchasing with NPCs.
 */
class buy : public fb::game::npc_interaction_handler
{
public:
    /**
     * @brief   Constructs a buy handler.
     *
     * @param[in]  server  Reference to the game server
     */
    buy(server_type& server) : npc_interaction_handler(server) {}

    /**
     * @brief   Checks if the message matches the buy pattern.
     *
    ...
    /**
     * @brief   Handles item purchasing interaction.
     *
...
#endif // FB_GAME_HANDLER_NPC_INTERACTION_BUY_H