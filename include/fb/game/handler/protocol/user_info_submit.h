 ...
 */
class user_info_submit : public fb::handler::protocol<fb::game::server, game_reqs::user_info_submit>
{
public:
    user_info_submit(fb::game::server& server) :
        fb::handler::protocol<fb::game::server, game_reqs::user_info_submit>(server)
    { }
    ...
    async::task<bool> handle(fb::socket<character>& session, game_reqs::user_info_submit& request) override;
};
