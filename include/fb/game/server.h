...
public:
    ...
    [[nodiscard]] async::task<void> upsert_group_then(uint32_t gid, const std::function<void(group_ptr&)>& fn);
    [[nodiscard]] async::task<void> upsert_group_then(uint32_t                               gid,
    ...
    [[nodiscard]] async::task<void>
         upsert_clan_then(uint32_t id, std::function<async::task<void>(std::shared_ptr<fb::game::clan>&)> fn);
    void rezen_force();
...
public:
    void                            update_status();
    void                            update_time();
    [[nodiscard]] async::task<bool> npc_interaction(character&                                         ch,
