// <auto-generated>
//  automatically generated by the FlatBuffers compiler, do not modify
// </auto-generated>

namespace fb.protocol._internal.response.raw
{

using global::System;
using global::System.Collections.Generic;
using global::Google.FlatBuffers;

public struct SetClanTitle : IFlatbufferObject
{
  private Table __p;
  public ByteBuffer ByteBuffer { get { return __p.bb; } }
  public static void ValidateVersion() { FlatBufferConstants.FLATBUFFERS_24_3_25(); }
  public static SetClanTitle GetRootAsSetClanTitle(ByteBuffer _bb) { return GetRootAsSetClanTitle(_bb, new SetClanTitle()); }
  public static SetClanTitle GetRootAsSetClanTitle(ByteBuffer _bb, SetClanTitle obj) { return (obj.__assign(_bb.GetInt(_bb.Position) + _bb.Position, _bb)); }
  public static bool VerifySetClanTitle(ByteBuffer _bb) {Google.FlatBuffers.Verifier verifier = new Google.FlatBuffers.Verifier(_bb); return verifier.VerifyBuffer("", false, SetClanTitleVerify.Verify); }
  public void __init(int _i, ByteBuffer _bb) { __p = new Table(_i, _bb); }
  public SetClanTitle __assign(int _i, ByteBuffer _bb) { __init(_i, _bb); return this; }

  public uint Clan { get { int o = __p.__offset(4); return o != 0 ? __p.bb.GetUint(o + __p.bb_pos) : (uint)0; } }
  public string Title { get { int o = __p.__offset(6); return o != 0 ? __p.__string(o + __p.bb_pos) : null; } }
#if ENABLE_SPAN_T
  public Span<byte> GetTitleBytes() { return __p.__vector_as_span<byte>(6, 1); }
#else
  public ArraySegment<byte>? GetTitleBytes() { return __p.__vector_as_arraysegment(6); }
#endif
  public byte[] GetTitleArray() { return __p.__vector_as_array<byte>(6); }
  public uint Error { get { int o = __p.__offset(8); return o != 0 ? __p.bb.GetUint(o + __p.bb_pos) : (uint)0; } }

  public static Offset<fb.protocol._internal.response.raw.SetClanTitle> CreateSetClanTitle(FlatBufferBuilder builder,
      uint clan = 0,
      StringOffset titleOffset = default(StringOffset),
      uint error = 0) {
    builder.StartTable(3);
    SetClanTitle.AddError(builder, error);
    SetClanTitle.AddTitle(builder, titleOffset);
    SetClanTitle.AddClan(builder, clan);
    return SetClanTitle.EndSetClanTitle(builder);
  }

  public static void StartSetClanTitle(FlatBufferBuilder builder) { builder.StartTable(3); }
  public static void AddClan(FlatBufferBuilder builder, uint clan) { builder.AddUint(0, clan, 0); }
  public static void AddTitle(FlatBufferBuilder builder, StringOffset titleOffset) { builder.AddOffset(1, titleOffset.Value, 0); }
  public static void AddError(FlatBufferBuilder builder, uint error) { builder.AddUint(2, error, 0); }
  public static Offset<fb.protocol._internal.response.raw.SetClanTitle> EndSetClanTitle(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<fb.protocol._internal.response.raw.SetClanTitle>(o);
  }
  public static void FinishSetClanTitleBuffer(FlatBufferBuilder builder, Offset<fb.protocol._internal.response.raw.SetClanTitle> offset) { builder.Finish(offset.Value); }
  public static void FinishSizePrefixedSetClanTitleBuffer(FlatBufferBuilder builder, Offset<fb.protocol._internal.response.raw.SetClanTitle> offset) { builder.FinishSizePrefixed(offset.Value); }
}


static public class SetClanTitleVerify
{
  static public bool Verify(Google.FlatBuffers.Verifier verifier, uint tablePos)
  {
    return verifier.VerifyTableStart(tablePos)
      && verifier.VerifyField(tablePos, 4 /*Clan*/, 4 /*uint*/, 4, false)
      && verifier.VerifyString(tablePos, 6 /*Title*/, false)
      && verifier.VerifyField(tablePos, 8 /*Error*/, 4 /*uint*/, 4, false)
      && verifier.VerifyTableEnd(tablePos);
  }
}

}
