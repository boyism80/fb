// <auto-generated>
//  automatically generated by the FlatBuffers compiler, do not modify
// </auto-generated>

namespace fb.protocol._internal.response.raw
{

using global::System;
using global::System.Collections.Generic;
using global::Google.FlatBuffers;

public struct BroadcastGroup : IFlatbufferObject
{
  private Table __p;
  public ByteBuffer ByteBuffer { get { return __p.bb; } }
  public static void ValidateVersion() { FlatBufferConstants.FLATBUFFERS_24_3_25(); }
  public static BroadcastGroup GetRootAsBroadcastGroup(ByteBuffer _bb) { return GetRootAsBroadcastGroup(_bb, new BroadcastGroup()); }
  public static BroadcastGroup GetRootAsBroadcastGroup(ByteBuffer _bb, BroadcastGroup obj) { return (obj.__assign(_bb.GetInt(_bb.Position) + _bb.Position, _bb)); }
  public static bool VerifyBroadcastGroup(ByteBuffer _bb) {Google.FlatBuffers.Verifier verifier = new Google.FlatBuffers.Verifier(_bb); return verifier.VerifyBuffer("", false, BroadcastGroupVerify.Verify); }
  public void __init(int _i, ByteBuffer _bb) { __p = new Table(_i, _bb); }
  public BroadcastGroup __assign(int _i, ByteBuffer _bb) { __init(_i, _bb); return this; }

  public uint Host { get { int o = __p.__offset(4); return o != 0 ? __p.bb.GetUint(o + __p.bb_pos) : (uint)0; } }
  public uint Group { get { int o = __p.__offset(6); return o != 0 ? __p.bb.GetUint(o + __p.bb_pos) : (uint)0; } }
  public string Message { get { int o = __p.__offset(8); return o != 0 ? __p.__string(o + __p.bb_pos) : null; } }
#if ENABLE_SPAN_T
  public Span<byte> GetMessageBytes() { return __p.__vector_as_span<byte>(8, 1); }
#else
  public ArraySegment<byte>? GetMessageBytes() { return __p.__vector_as_arraysegment(8); }
#endif
  public byte[] GetMessageArray() { return __p.__vector_as_array<byte>(8); }
  public byte Type { get { int o = __p.__offset(10); return o != 0 ? __p.bb.Get(o + __p.bb_pos) : (byte)0; } }
  public uint Error { get { int o = __p.__offset(12); return o != 0 ? __p.bb.GetUint(o + __p.bb_pos) : (uint)0; } }

  public static Offset<fb.protocol._internal.response.raw.BroadcastGroup> CreateBroadcastGroup(FlatBufferBuilder builder,
      uint host = 0,
      uint group = 0,
      StringOffset messageOffset = default(StringOffset),
      byte type = 0,
      uint error = 0) {
    builder.StartTable(5);
    BroadcastGroup.AddError(builder, error);
    BroadcastGroup.AddMessage(builder, messageOffset);
    BroadcastGroup.AddGroup(builder, group);
    BroadcastGroup.AddHost(builder, host);
    BroadcastGroup.AddType(builder, type);
    return BroadcastGroup.EndBroadcastGroup(builder);
  }

  public static void StartBroadcastGroup(FlatBufferBuilder builder) { builder.StartTable(5); }
  public static void AddHost(FlatBufferBuilder builder, uint host) { builder.AddUint(0, host, 0); }
  public static void AddGroup(FlatBufferBuilder builder, uint group) { builder.AddUint(1, group, 0); }
  public static void AddMessage(FlatBufferBuilder builder, StringOffset messageOffset) { builder.AddOffset(2, messageOffset.Value, 0); }
  public static void AddType(FlatBufferBuilder builder, byte type) { builder.AddByte(3, type, 0); }
  public static void AddError(FlatBufferBuilder builder, uint error) { builder.AddUint(4, error, 0); }
  public static Offset<fb.protocol._internal.response.raw.BroadcastGroup> EndBroadcastGroup(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<fb.protocol._internal.response.raw.BroadcastGroup>(o);
  }
  public static void FinishBroadcastGroupBuffer(FlatBufferBuilder builder, Offset<fb.protocol._internal.response.raw.BroadcastGroup> offset) { builder.Finish(offset.Value); }
  public static void FinishSizePrefixedBroadcastGroupBuffer(FlatBufferBuilder builder, Offset<fb.protocol._internal.response.raw.BroadcastGroup> offset) { builder.FinishSizePrefixed(offset.Value); }
}


static public class BroadcastGroupVerify
{
  static public bool Verify(Google.FlatBuffers.Verifier verifier, uint tablePos)
  {
    return verifier.VerifyTableStart(tablePos)
      && verifier.VerifyField(tablePos, 4 /*Host*/, 4 /*uint*/, 4, false)
      && verifier.VerifyField(tablePos, 6 /*Group*/, 4 /*uint*/, 4, false)
      && verifier.VerifyString(tablePos, 8 /*Message*/, false)
      && verifier.VerifyField(tablePos, 10 /*Type*/, 1 /*byte*/, 1, false)
      && verifier.VerifyField(tablePos, 12 /*Error*/, 4 /*uint*/, 4, false)
      && verifier.VerifyTableEnd(tablePos);
  }
}

}
