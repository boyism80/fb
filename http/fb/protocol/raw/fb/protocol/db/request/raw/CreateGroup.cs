// <auto-generated>
//  automatically generated by the FlatBuffers compiler, do not modify
// </auto-generated>

namespace fb.protocol.db.request.raw
{

using global::System;
using global::System.Collections.Generic;
using global::Google.FlatBuffers;

public struct CreateGroup : IFlatbufferObject
{
  private Table __p;
  public ByteBuffer ByteBuffer { get { return __p.bb; } }
  public static void ValidateVersion() { FlatBufferConstants.FLATBUFFERS_24_3_25(); }
  public static CreateGroup GetRootAsCreateGroup(ByteBuffer _bb) { return GetRootAsCreateGroup(_bb, new CreateGroup()); }
  public static CreateGroup GetRootAsCreateGroup(ByteBuffer _bb, CreateGroup obj) { return (obj.__assign(_bb.GetInt(_bb.Position) + _bb.Position, _bb)); }
  public static bool VerifyCreateGroup(ByteBuffer _bb) {Google.FlatBuffers.Verifier verifier = new Google.FlatBuffers.Verifier(_bb); return verifier.VerifyBuffer("", false, CreateGroupVerify.Verify); }
  public void __init(int _i, ByteBuffer _bb) { __p = new Table(_i, _bb); }
  public CreateGroup __assign(int _i, ByteBuffer _bb) { __init(_i, _bb); return this; }

  public uint Master { get { int o = __p.__offset(4); return o != 0 ? __p.bb.GetUint(o + __p.bb_pos) : (uint)0; } }
  public uint Member { get { int o = __p.__offset(6); return o != 0 ? __p.bb.GetUint(o + __p.bb_pos) : (uint)0; } }
  public byte Host { get { int o = __p.__offset(8); return o != 0 ? __p.bb.Get(o + __p.bb_pos) : (byte)0; } }

  public static Offset<fb.protocol.db.request.raw.CreateGroup> CreateCreateGroup(FlatBufferBuilder builder,
      uint master = 0,
      uint member = 0,
      byte host = 0) {
    builder.StartTable(3);
    CreateGroup.AddMember(builder, member);
    CreateGroup.AddMaster(builder, master);
    CreateGroup.AddHost(builder, host);
    return CreateGroup.EndCreateGroup(builder);
  }

  public static void StartCreateGroup(FlatBufferBuilder builder) { builder.StartTable(3); }
  public static void AddMaster(FlatBufferBuilder builder, uint master) { builder.AddUint(0, master, 0); }
  public static void AddMember(FlatBufferBuilder builder, uint member) { builder.AddUint(1, member, 0); }
  public static void AddHost(FlatBufferBuilder builder, byte host) { builder.AddByte(2, host, 0); }
  public static Offset<fb.protocol.db.request.raw.CreateGroup> EndCreateGroup(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<fb.protocol.db.request.raw.CreateGroup>(o);
  }
  public static void FinishCreateGroupBuffer(FlatBufferBuilder builder, Offset<fb.protocol.db.request.raw.CreateGroup> offset) { builder.Finish(offset.Value); }
  public static void FinishSizePrefixedCreateGroupBuffer(FlatBufferBuilder builder, Offset<fb.protocol.db.request.raw.CreateGroup> offset) { builder.FinishSizePrefixed(offset.Value); }
}


static public class CreateGroupVerify
{
  static public bool Verify(Google.FlatBuffers.Verifier verifier, uint tablePos)
  {
    return verifier.VerifyTableStart(tablePos)
      && verifier.VerifyField(tablePos, 4 /*Master*/, 4 /*uint*/, 4, false)
      && verifier.VerifyField(tablePos, 6 /*Member*/, 4 /*uint*/, 4, false)
      && verifier.VerifyField(tablePos, 8 /*Host*/, 1 /*byte*/, 1, false)
      && verifier.VerifyTableEnd(tablePos);
  }
}

}
